...
import {
  ...
  Gem, Pin, PinOff, GripVertical, Bell,
} from 'lucide-react';
...
function AsteriskIcon({ size, color }) {
  ...
  return (
    <svg width={s} height={s} viewBox={'0 0 ' + s + ' ' + s} fill="none">
      {lines.map((l, i) => (
        <line key={i} x1={l.x1} y1={l.y1} x2={l.x2} y2={l.y2} stroke={color || DEFAULT} strokeWidth="2.5" strokeLinecap="round" />
      ))}
    </svg>
  );
}
    ...
    >
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'20px 24px', display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:20 }}>
        {CREATE_COLUMNS.map(function(col) {
          return (
            <div key={col.header}>
              <div style={{ fontSize:13, fontWeight:700, color:'#0A2540', marginBottom:14, paddingBottom:8, borderBottom:'1px solid #F1F5F9' }}>
                {col.header}
              </div>
...
function FlyoutPanel({ title, onClose, children, width, onMouseLeave }) {
  const w = width || 280;
  return (
    <div
...
function CustomizeModal({ onClose, pinnedApps, appOrder, onSave }) {
  ...
  const reset = function() {
    setOrder(DEFAULT_APP_ORDER);
    setPinned(['accounting']);
  };
    ...
    >
      ...
      <div style={{ background:'#fff', borderRadius:14, width:'100%', maxWidth:520, maxHeight:'90vh', display:'flex', flexDirection:'column', boxShadow:'0 24px 60px rgba(0,0,0,0.2)', animation:'fadeIn 0.2s ease', overflow:'hidden' }}>
        ...
        <div style={{ padding:'24px 24px 16px', borderBottom:'1px solid #F1F5F9', display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexShrink:0 }}>
          ...
          <button onClick={onClose} style={{ background:'none', border:'none', cursor:'pointer', color:MUTED, display:'flex', padding:4, marginLeft:16, flexShrink:0 }}>
            <X size={20}/>
          </button>
        </div>
        ...
        <div style={{ padding:'16px 24px', borderTop:'1px solid #F1F5F9', display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}>
          <button
            onClick={reset}
            style={{ background:'none', border:'none', cursor:'pointer', fontSize:13, color:MUTED, fontFamily:FONT, textDecoration:'underline' }}
          >
            Reset to default
          </button>
          <div style={{ display:'flex', gap:10 }}>
            <button
              onClick={onClose}
              style={{ padding:'9px 20px', borderRadius:8, background:'#fff', border:'1px solid #E2E8F0', cursor:'pointer', fontSize:13, fontWeight:500, color:MUTED, fontFamily:FONT }}
            >
              Cancel
            </button>
            <button
              onClick={function() { onSave(pinned, order); onClose(); }}
              style={{ padding:'9px 20px', borderRadius:8, background:ACCENT, border:'none', cursor:'pointer', fontSize:13, fontWeight:600, color:'#fff', fontFamily:FONT }}
            >
              Save
            </button>
          </div>
...
export default function Sidebar({ onLogout, mobileOpen, onMobileClose, isMobile }) {
  ...
  useEffect(function() {
    const handler = function(e) {
      if (e.key === 'Escape') { setFlyout(null); setShowCustomize(false); setFlyoutPinned(false); }
    };
  ...
  const startFlyoutHover = function(flyoutId, delay) {
    clearTimeout(closeTimers.current[flyoutId]);
    hoverTimers.current[flyoutId] = setTimeout(function() {
      if (!flyoutPinned) setFlyout(flyoutId);
    }, delay || 300);
  };

  const endFlyoutHover = function(flyoutId) {
    clearTimeout(hoverTimers.current[flyoutId]);
    if (!flyoutPinned) {
      closeTimers.current[flyoutId] = setTimeout(function() {
        setFlyout(function(prev) { return prev === flyoutId ? null : prev; });
      }, 200);
    }
  };

  const keepFlyout = function(flyoutId) {
    clearTimeout(closeTimers.current[flyoutId]);
  };
    ...
    return (
      <div
        ...
        onMouseEnter={function(e) {
          setHovItem(item.id);
          if (item.flyout) startFlyoutHover(item.flyout, 300);
        }}
        onMouseLeave={function() {
          setHovItem(null);
          if (item.flyout) endFlyoutHover(item.flyout);
        }}
    ...
    <div ref={sidebarRef}>
      ...
      <aside style={{ position:'fixed', top:56, left:0, width:80, height:'calc(100vh - 56px)', background:'#FAFAFA', borderRight:'1px solid #E8EDF3', display:mobile?'none':'flex', flexDirection:'column', alignItems:'center', paddingTop:8, paddingBottom:8, zIndex:44, fontFamily:FONT, overflowY:'auto', scrollbarWidth:'none' }}>
        ...
        <div style={{ width:'100%', display:'flex', flexDirection:'column', alignItems:'center', padding:'10px 0 4px' }}>
          <div style={{ fontSize:9, fontWeight:600, color:'#475569', letterSpacing:'0.08em', textTransform:'uppercase', textAlign:'center', marginBottom:6 }}>
            PINNED
          </div>
      ...
      {!mobile && flyout === 'bookmarks' && (
        <FlyoutPanel
          title="Bookmarks"
          onClose={function() { setFlyout(null); setFlyoutPinned(false); }}
          onMouseLeave={function() { if (!flyoutPinned) endFlyoutHover('bookmarks'); }}
        >
          <div style={{ padding:'24px 20px', textAlign:'center' }}>
            ...
            <button
              onClick={function() { setFlyout(null); }}
      ...
      {!mobile && flyout === 'feed' && (
        <FlyoutPanel title="Feed" onClose={function() { setFlyout(null); setFlyoutPinned(false); }}>
          <div style={{ padding:'24px 20px', textAlign:'center' }}>
      ...
      {!mobile && flyout === 'reports' && (
        <FlyoutPanel title="Reports" onClose={function() { setFlyout(null); setFlyoutPinned(false); }}>
          <div style={{ padding:'8px 12px' }}>
            <div style={{ fontSize:9, fontWeight:700, color:MUTED, letterSpacing:'0.14em', textTransform:'uppercase', padding:'10px 8px 8px' }}>REPORTS</div>
            {[
              { label:'Standard reports',    path:'/reports'                      },
              { label:'Custom reports',      path:'/reports/custom'               },
              { label:'Management reports',  path:'/reports/management'           },
              { label:'Cash flow overview',  path:'/reports/cash-flow-overview'   },
              { label:'Cash flow planner',   path:'/reports/cash-flow-planner'    },
            ].map(function(item) {
              return (
                <div
                  key={item.label}
                  onClick={function() { goTo(item.path); }}
                  style={{ display:'flex', alignItems:'center', gap:10, padding:'11px 12px', borderRadius:8, cursor:'pointer', marginBottom:2, transition:'all 0.15s' }}
                  onMouseEnter={function(e) { e.currentTarget.style.background='#F1F5F9'; }}
                  onMouseLeave={function(e) { e.currentTarget.style.background='transparent'; }}
                >
      ...
      {!mobile && flyout === 'more' && (
        <FlyoutPanel title="More" onClose={function() { setFlyout(null); setFlyoutPinned(false); }}>
          <div style={{ padding:'8px 12px' }}>
            <div style={{ fontSize:9, fontWeight:700, color:MUTED, letterSpacing:'0.14em', textTransform:'uppercase', padding:'10px 8px 8px' }}>MORE</div>
            {ALL_APPS.filter(function(a) { return !pinnedApps.includes(a.id); }).map(function(app) {
              const Icon = app.icon;
              return (
                <div
                  key={app.id}
                  onClick={function() { setFlyout('apps'); setFlyoutExpanded(app.id); }}
