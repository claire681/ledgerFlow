  ...
  return (
    <img
      ...
      style={{
        ...
        display: "block",
        flexShrink: 0,
        boxShadow: "0 0 0 1px rgba(14,26,26,0.08)",
      }}
      loading="lazy"
    />
  );
  ...
  return (
    <div ref={ref} style={{ position: "relative", flexShrink: 0 }}>
      <button type="button" onClick={() => setOpen(!open)}
        style={{ display: "flex", alignItems: "center", gap: 8, background: BG, border: "1px solid " + BORDER, borderRadius: 10, padding: "12px 10px 12px 12px", cursor: "pointer", fontFamily: "inherit", fontSize: 14, color: INK, minWidth: 115, transition: "border-color 0.15s" }}
        onMouseEnter={e => e.currentTarget.style.borderColor = SUB}
        onMouseLeave={e => e.currentTarget.style.borderColor = BORDER}>
        <Flag iso2={selected.iso2} size={16} />
              ...
              filtered.map((c) => {
                ...
                return (
                  <button key={c.iso2 + c.dial} type="button" onClick={() => pick(c)}
                    ...
                    style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "10px 14px", background: isSel ? "rgba(15,149,153,0.06)" : "transparent", border: "none", cursor: "pointer", fontFamily: "inherit", fontSize: 14, color: INK, textAlign: "left", transition: "background 0.1s" }}>
