...
export default function InvoicePreview({ inv, customization, accentColor, template, paymentOptions, scheduling, printerFriendly, selectedFont, onFieldChange, onCustomerSelect, onItemChange, onAddItem, onDeleteItem, onClearItems, onEditCompany , onEditCustomer , onEditTotals }) {
 ...
 {itemsEditable && <th style={{ width: 20, padding: "10px 2px" }}></th>}
    ...
    if (rowType === "subtotal") {
      ...
      return (
        <tr key={i} onMouseEnter={() => setHoveredRow(i)} onMouseLeave={() => setHoveredRow(null)} style={{ borderBottom: "1px solid #f1f5f9", position: "relative" }}>
          ...
          {itemsEditable && (<td className="print-hide" style={{ width: 20, padding: "4px 2px", verticalAlign: "middle", textAlign: "center", color: hovered ? "#94a3b8" : "transparent", cursor: "grab", fontSize: 14, lineHeight: 1, userSelect: "none" }}>⠿</td>)}
          ...
          {itemsEditable && (<td className="print-hide" style={{ padding: "8px 4px", textAlign: "center", verticalAlign: "middle" }}><button onClick={() => onDeleteItem(i)} style={{ background: "none", border: "none", cursor: "pointer", padding: 4, display: "inline-flex", borderRadius: 4, opacity: hovered ? 1 : 0, transition: "opacity 0.12s" }}><Trash2 size={14} color="#94a3b8" /></button></td>)}
        </tr>
      );
    }
    ...
    return (
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12, alignItems: "center", flexWrap: "wrap" }}>
        ...
        {items.length > 0 && (
          <button className="print-hide" onClick={() => { if (window.confirm("Clear all line items? This cannot be undone.")) onClearItems(); }} style={{ padding: "8px 14px", background: "#f1f5f9", border: "1px solid #e2e8f0", borderRadius: 6, color: "#0F172A", fontSize: 13, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>Clear all lines</button>
        )}
      </div>
    )}
