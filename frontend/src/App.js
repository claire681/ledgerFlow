...
export default function Sidebar({ onLogout, onNavigate }) {
  ...
  const bottomItems = [
    { label: 'Settings', icon: SettingsIcon, path: '/settings', onClick: () => { navigate('/settings'); onNavigate && onNavigate(); } },
    { label: 'Help',     icon: HelpCircle,   path: '/help',     onClick: () => { navigate('/help');     onNavigate && onNavigate(); } },
  ];

  return (
    <aside style={{ width: 260, minWidth: 260, height: '100vh', background: 'linear-gradient(180deg, #0F172A 0%, #0D1526 100%)', display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative', fontFamily: FONT, boxShadow: '4px 0 32px rgba(0,0,0,0.25)' }}>

      {/* Top gradient line */}
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: GRAD, zIndex: 1 }} />

      {/* Header */}
      <div style={{ padding: '20px 20px 18px', borderBottom: `1px solid ${BORDER}`, marginTop: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 40, height: 40, borderRadius: 11, background: GRAD, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 4px 16px rgba(10,185,138,0.3)' }}>
            <LogoIcon />
          </div>
          <div>
      ...
      {/* User card */}
      <div style={{ margin: '14px 14px 4px', padding: '12px 14px', background: 'rgba(255,255,255,0.03)', border: `1px solid ${BORDER}`, borderRadius: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 38, height: 38, borderRadius: 10, background: GRAD, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 700, color: '#0F172A', flexShrink: 0 }}>
          {initial}
        </div>
      ...
      {/* Navigation */}
      <nav style={{ flex: 1, overflowY: 'auto', padding: '4px 10px 4px', scrollbarWidth: 'none' }}>
        {NAV.map(group => (
            ...
            {group.items.map(item => {
              ...
              return (
                <div
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 10, cursor: 'pointer', marginBottom: 2, background: isActive ? ACTIVE : isHov ? HOVER : 'transparent', borderLeft: `3px solid ${isActive ? ACCENT : 'transparent'}`, transition: 'all 0.15s ease' }}
                >
                  <Icon size={18} color={isActive ? ACCENT : isHov ? TEXT_SUB : TEXT_DIM} strokeWidth={isActive ? 2.5 : 1.8} style={{ flexShrink: 0 }} />
                  ...
                  {item.badge && (
                    <span style={{ fontSize: 8, fontWeight: 700, color: item.badge === 'NEW' ? '#fff' : ACCENT, background: item.badge === 'NEW' ? ACCENT : ACTIVE, border: `1px solid ${item.badge === 'SMART' ? ACCENT + '35' : 'transparent'}`, padding: '2px 7px', borderRadius: 20, letterSpacing: '0.05em' }}>
                      {item.badge}
                    </span>
                  )}
                </div>
      ...
      <div style={{ padding: '8px 14px 24px' }}>
        ...
        {bottomItems.map(item => {
          ...
          return (
            <div
              ...
              onMouseLeave={() => setHov(null)}
              style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 10, cursor: 'pointer', marginBottom: 4, background: isActive ? ACTIVE : isHov ? HOVER : 'transparent', borderLeft: `3px solid ${isActive ? ACCENT : 'transparent'}`, transition: 'all 0.15s ease' }}
            >
              <Icon size={18} color={isActive ? ACCENT : isHov ? TEXT_SUB : TEXT_DIM} strokeWidth={1.8} style={{ flexShrink: 0 }} />
        ...
        <button
          ...
          style={{ width: '100%', padding: '10px 14px', borderRadius: 10, cursor: 'pointer', fontSize: 13, fontWeight: 500, color: TEXT_DIM, background: 'transparent', border: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.15s', fontFamily: FONT, marginTop: 4 }}
        >
