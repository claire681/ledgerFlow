      ...
      <div style={{ flex:1, overflow:'auto', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
        ...
        {error && (
          <div style={{ color:'#EF4444', textAlign:'center' }}>
            ...
            <div style={{ fontSize:12, color:'#94A3B8' }}>File may not be available for preview</div>
          </div>
        )}
        {!loading && !error && url && (
          <>
            {isImage && (
              <img src={url} alt={doc.filename} style={{ maxWidth:'100%', maxHeight:'80vh', objectFit:'contain', transform:`scale(${zoom}) rotate(${rotate}deg)`, transition:'transform 0.2s ease', borderRadius:8, boxShadow:'0 8px 32px rgba(0,0,0,0.4)' }}/>
            )}
            {isPDF && (
