  ...
  return (
    <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
      ...
      <circle
        ...
        fill="none" stroke="url(#pgrd)" strokeWidth={8}
...
export default function Documents() {
  ...
  const handleUpload = async (file, txnType = 'auto') => {
    ...
    if (!token) { setError('Not logged in. Please sign in again.'); return; }
  ...
  const handleMarkPaid = async (doc) => {
    ...
    try {
      ...
      setDocs(prev => prev.map(d =>
        d.id === doc.id ? { ...d, payment_status: 'paid', paid_at: data.paid_at } : d
      ));
    ...
    <div style={page}>
      <style>{`
        @keyframes pulse-ring {
          0%   { box-shadow: 0 0 0 0 rgba(10,185,138,0.4); }
          70%  { box-shadow: 0 0 0 12px rgba(10,185,138,0); }
          100% { box-shadow: 0 0 0 0 rgba(10,185,138,0); }
        }
        @keyframes float {
      ...
      <div style={topBar}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 700, color: L.text, letterSpacing: '-0.02em' }}>Documents</div>
          <div style={{ fontSize: 12, color: L.textMuted, marginTop: 2 }}>
            Upload invoices and receipts — AI extracts data automatically
          </div>
        </div>
      ...
      <div style={{ padding: '24px 28px' }}>
        ...
        {/* ── Alerts ── */}
        {success && (
          <div style={{ padding: '12px 16px', borderRadius: L.radiusSm, background: 'rgba(10,185,138,0.08)', border: '1px solid rgba(10,185,138,0.2)', color: ACCENT, fontSize: 13, marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', animation: 'fadeIn 0.3s ease' }}>
        ...
        {error && (
          <div style={{ padding: '12px 16px', borderRadius: L.radiusSm, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', color: '#EF4444', fontSize: 13, marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', animation: 'fadeIn 0.3s ease' }}>
            ...
            <div style={{ animation: 'fadeIn 0.4s ease' }}>
              <div style={{ position: 'relative', width: 120, height: 120, margin: '0 auto 20px' }}>
                ...
                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ width: 56, height: 56, borderRadius: 16, background: GRAD, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(10,185,138,0.35)', animation: 'pulse-ring 2s infinite' }}>
                    <Sparkles size={24} color="#fff" />
                  </div>
                </div>
              </div>
              <div style={{ fontSize: 16, fontWeight: 700, color: L.text, marginBottom: 4 }}>AI is processing your document</div>
              <div style={{ fontSize: 13, color: L.textMuted, marginBottom: 6 }}>{uploadStatus?.filename || ''}</div>
              <div style={{ width: 320, height: 6, background: '#E2E8F0', borderRadius: 99, margin: '0 auto 24px', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'inline-flex', flexDirection: 'column', gap: 10, textAlign: 'left' }}>
                {PROCESS_STEPS.map((step, index) => {
                  ...
                  return (
                    <div key={step.key} style={{ display: 'flex', alignItems: 'center', gap: 10, opacity: done || active ? 1 : 0.35, transition: 'opacity 0.3s ease' }}>
                      <div style={{ width: 28, height: 28, borderRadius: 8, flexShrink: 0, background: done ? 'rgba(10,185,138,0.12)' : active ? `${step.color}18` : '#F1F5F9', border: `1px solid ${done ? 'rgba(10,185,138,0.3)' : active ? `${step.color}35` : '#E2E8F0'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.3s ease' }}>
                        {done ? <CheckCircle size={13} color={ACCENT} /> : active ? <Spinner size={13} color={step.color} /> : <Icon size={13} color="#CBD5E1" />}
                      </div>
                      <span style={{ fontSize: 13, fontWeight: active ? 600 : done ? 500 : 400, color: done ? L.text : active ? step.color : L.textMuted, transition: 'color 0.3s ease' }}>{step.label}</span>
                      {active && <div style={{ fontSize: 10, fontWeight: 700, color: step.color, background: `${step.color}15`, border: `1px solid ${step.color}30`, padding: '1px 7px', borderRadius: 20 }}>RUNNING</div>}
                      {done   && <div style={{ fontSize: 10, fontWeight: 700, color: ACCENT, background: 'rgba(10,185,138,0.1)', border: '1px solid rgba(10,185,138,0.25)', padding: '1px 7px', borderRadius: 20 }}>DONE</div>}
                    </div>
                  );
                })}
              </div>
              <div style={{ fontSize: 28, fontWeight: 800, color: L.text, marginTop: 20, letterSpacing: '-0.03em' }}>{progress}%</div>
            </div>
          ) : (
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              <div style={{ width: 64, height: 64, borderRadius: 18, background: 'rgba(10,185,138,0.08)', border: '1px solid rgba(10,185,138,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px', animation: 'float 3s ease-in-out infinite' }}>
                <Upload size={26} color={ACCENT} />
              </div>
              <div style={{ fontSize: 16, fontWeight: 700, color: L.text, marginBottom: 6 }}>{dragOver ? 'Drop to upload' : 'Drop files here or click to browse'}</div>
              <div style={{ fontSize: 13, color: L.textMuted, marginBottom: 20 }}>PDF, CSV, PNG, JPG — invoices, receipts, bank statements</div>
        ...
        <div style={{ ...card, overflow: 'hidden' }}>
          ...
          {/* Card header */}
          <div style={{ padding: '16px 20px', borderBottom: `1px solid ${L.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
            ...
            {docs.length > 0 && (
              <div style={{ fontSize: 11, color: L.textMuted }}>
                Total value: <strong style={{ color: L.text }}>${docs.reduce((s, d) => s + (d.total_amount || 0), 0).toLocaleString()}</strong>
              </div>
            )}
          </div>
          ...
          {/* Search bar */}
          {docs.length > 0 && (
            <div style={{ padding: '12px 20px', borderBottom: `1px solid ${L.border}`, display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ position: 'relative', flex: 1 }}>
                ...
                <input
                  type="text"
                  placeholder="Search by file name, vendor, client, type, status, amount..."
                  value={search}
                  onChange={e => setSearch(e.target.value)}
                  style={{ width: '100%', padding: '8px 32px 8px 32px', background: L.pageBg, border: `1px solid ${L.border}`, borderRadius: L.radiusSm, color: L.text, fontSize: 13, fontFamily: L.font, outline: 'none', boxSizing: 'border-box', transition: 'border-color 0.15s' }}
              ...
              {search && <div style={{ fontSize: 12, color: L.textMuted, whiteSpace: 'nowrap' }}>{filteredDocs.length} of {docs.length}</div>}
            </div>
          )}

          {/* Table header — STATUS column is wider to fit both badges */}
          {docs.length > 0 && (
          ...
          {!loading && docs.length > 0 && filteredDocs.length === 0 && (
            <div style={{ padding: '40px 20px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 13, color: L.textMuted, marginBottom: 14 }}>No results matching "<strong>{search}</strong>"</div>
          ...
          {filteredDocs.map((doc, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Type */}
                <div>
                  <span style={{ fontSize: 10, fontWeight: 700, color: dts.color, background: `${dts.color}12`, padding: '3px 8px', borderRadius: 20, border: `1px solid ${dts.color}25` }}>
                    {dts.label}
                  </span>
                </div>

                {/* Vendor */}
                <div style={{ fontSize: 13, color: L.textSub, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {doc.vendor || '—'}
                </div>
                ...
                {/* ── Dual status badges — processing + payment ── */}
                ...
                {/* ── Actions — View, Mark as Paid, Download, Delete ── */}
                <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexWrap: 'wrap' }}>
                  {doc.has_file && (
                    <button
                      onClick={() => setViewingDoc(doc)}
                      style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 9px', borderRadius: L.radiusSm, background: 'rgba(14,165,233,0.08)', border: '1px solid rgba(14,165,233,0.2)', color: '#0EA5E9', cursor: 'pointer', fontSize: 11, fontWeight: 600, fontFamily: L.font, transition: 'all 0.15s' }}
                  ...
                  {/* Mark as Paid — shows for any due/overdue/partially_paid doc */}
                  {needsMarkPaid(doc) && (
                    <button
                      onClick={() => handleMarkPaid(doc)}
                      disabled={isPaying}
                      style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 9px', borderRadius: L.radiusSm, background: isPaying ? L.pageBg : 'rgba(10,185,138,0.08)', border: '1px solid rgba(10,185,138,0.3)', color: ACCENT, cursor: isPaying ? 'not-allowed' : 'pointer', fontSize: 11, fontWeight: 600, fontFamily: L.font, transition: 'all 0.15s' }}
                  ...
                  {doc.has_file && (
                    <button
                      ...
                      style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 7px', borderRadius: L.radiusSm, background: 'transparent', border: `1px solid ${L.border}`, color: L.textMuted, cursor: 'pointer', fontSize: 11, transition: 'all 0.15s' }}
      ...
      {showTypeModal && pendingFile && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }}>
          <div style={{ background: '#FFFFFF', borderRadius: 20, padding: 32, maxWidth: 420, width: '90%', boxShadow: '0 24px 60px rgba(0,0,0,0.18)', animation: 'fadeIn 0.25s ease' }}>
            ...
            <div style={{ fontSize: 13, color: L.textMuted, textAlign: 'center', marginBottom: 24 }}>{pendingFile.name}</div>
