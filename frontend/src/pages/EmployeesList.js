...
const PRIVATE_MASK = (
  <span style={{ display: "inline-flex", alignItems: "center", color: "#12262B" }}>
  ...
  return (
    <div style={{ background: BG_CARD, minHeight: "100vh", width: "100%", fontFamily: "inherit" }}>
      <div style={{ padding: "24px 28px 36px", width: "100%" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20, marginBottom: 14, flexWrap: "wrap" }}><div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}><div style={{ width: searchFocused ? 420 : 260, height: 42, display: "flex", alignItems: "center", gap: 8, border: searchFocused ? "1px solid " + BRAND_DARK : "1px solid " + BORDER, borderRadius: 999, padding: "0 16px", fontSize: 14, background: "#FFFFFF", transition: "width 0.25s, border-color 0.15s" }}><input value={search} onChange={(e) => setSearch(e.target.value)} onFocus={() => setSearchFocused(true)} onBlur={() => setSearchFocused(false)} placeholder="Find an employee" style={{ flex: 1, border: "none", outline: "none", fontFamily: "inherit", fontSize: 14, fontWeight: 500, color: TEXT_PRIMARY, background: "transparent" }} /><Search size={16} style={{ color: "#94A0B2" }} /></div><div ref={activeMenuRef} style={{ position: "relative" }}><button onClick={() => setActiveMenuOpen(!activeMenuOpen)} style={{ display: "inline-flex", alignItems: "center", gap: 8, fontSize: 14, fontWeight: 600, color: TEXT_INK, padding: "0 18px", height: 42, border: activeMenuOpen ? "1px solid " + BRAND : "1px solid " + BORDER, borderRadius: 999, cursor: "pointer", background: activeMenuOpen ? "#E1F5EE" : "#FFFFFF", fontFamily: "inherit", whiteSpace: "nowrap" }}>{activeFilter} <ChevronDown size={16} strokeWidth={2} style={{ transform: activeMenuOpen ? "rotate(180deg)" : "none", transition: "transform 0.15s" }} /></button>{activeMenuOpen && (<div style={{ position: "absolute", top: 44, left: 0, background: BG_CARD, border: "0.5px solid " + BORDER, borderRadius: 8, boxShadow: "0 4px 12px rgba(0,0,0,0.08)", padding: 4, minWidth: 210, zIndex: 50 }}>{["Active Employees", "Inactive Employees", "All Employees"].map(opt => (<div key={opt} onClick={() => { setActiveFilter(opt); setActiveMenuOpen(false); }} onMouseEnter={(e) => { if (activeFilter !== opt) e.currentTarget.style.background = "#F8F9FA"; }} onMouseLeave={(e) => { if (activeFilter !== opt) e.currentTarget.style.background = "transparent"; }} style={{ display: "flex", alignItems: "center", height: 40, padding: "0 18px", cursor: "pointer", fontSize: 14, color: TEXT_INK, fontWeight: activeFilter === opt ? 600 : 500, background: activeFilter === opt ? "#E1F5EE" : "transparent", whiteSpace: "nowrap" }}>{opt}</div>))}</div>)}</div></div><div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}><span onClick={togglePrivacy} style={{ display: "inline-flex", alignItems: "center", gap: 8, fontSize: 13.5, fontWeight: 700, color: TEXT_INK, cursor: "pointer", padding: "9px 14px", border: "1.5px solid " + TEXT_INK, borderRadius: 9, background: BG_CARD }}><span style={{ width: 38, height: 22, borderRadius: 20, background: privacy ? BRAND : BORDER, position: "relative", transition: "0.15s" }}><span style={{ position: "absolute", top: 2, left: privacy ? 18 : 2, width: 18, height: 18, borderRadius: "50%", background: "white", boxShadow: "0 1px 2px rgba(0,0,0,0.2)", transition: "0.15s" }} /></span>Privacy</span><button onClick={() => navigate("/payroll/items")} style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 13.5, fontWeight: 600, color: TEXT_PRIMARY, padding: "9px 13px", border: "1.5px solid " + TEXT_INK, borderRadius: 9, cursor: "pointer", background: BG_CARD, fontFamily: "inherit", whiteSpace: "nowrap", fontWeight: 700 }}><ListOrdered size={16} />Payroll items</button><button onClick={() => navigate("/payroll/employees/add")} style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 13.5, fontWeight: 700, color: "#fff", padding: "9px 16px", border: 0, borderRadius: 9, cursor: "pointer", background: TEXT_INK, fontFamily: "inherit", whiteSpace: "nowrap" }}><UserPlus size={16} />Add employee</button></div></div>
