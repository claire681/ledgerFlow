...
export default function PayrollPreview() {
  ...
  const handleSubmitPayroll = async () => {
    if (finalizing || submitDisabled) return;
    setFinalizing(true);
    const token = getToken();
    try {
      ...
      navigate("/payroll/run/" + payRunId + "/done", {
        state: {
          runData: finalizedRun,
          stubs: lines,
        }
      });
    } catch (e) {
      console.error("Failed to finalize payroll", e);
  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      ...
      try {
        ...
        const passedStubs = Array.isArray(passedState.stubs) ? passedState.stubs : (passedCalc && Array.isArray(passedCalc.stubs) ? passedCalc.stubs : null);
      const calcLines = passedStubs || (passedCalc && (passedCalc.lines || passedCalc.employee_lines || passedCalc.employees || passedCalc.results || passedCalc.line_items));
        ...
        const mapApiLine = (l) => ({
          ...
          total_hours: Number(l.total_hours || l.hours || l.hours_total || (Number(l.hours_regular || 0) + Number(l.hours_overtime || 0) + Number(l.hours_stat_holiday || 0) + Number(l.hours_vacation || 0) + Number(l.hours_sick || 0)) || 0),
