...
export default function Currency() {
  ...
  // Set currency page context on mount — empty deps to avoid infinite loop
  useEffect(() => {
    setPageContext('currency', { page: 'currency' });
  }, []);
      ...
      <div style={{ position: 'relative' }} onClick={e => e.stopPropagation()}>
        <div
          ...
          style={{
            ...
            border: `1px solid ${show ? L.accentBorder : L.border}`,
        ...
        {show && (
          <div style={{
            ...
            borderRadius: L.radiusSm, zIndex: 100, marginTop: 4,
            boxShadow: L.shadowMd, maxHeight: 280, overflow: 'hidden',
            ...
            <div style={{ padding: 8 }}>
              <input autoFocus placeholder="Search..."
                ...
                style={{
                  ...
                  borderRadius: L.radiusSm, color: L.text, fontSize: 12,
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {filtered.map(c => (
                <div key={c.code}
                  onClick={() => { onSelect(c.code); setSearch(''); setShow(false); }}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 10,
                    padding: '9px 14px', cursor: 'pointer',
                    background: c.code === value ? L.accentSoft : 'transparent',
    ...
    <div style={page} onClick={() => { setShowFrom(false); setShowTo(false); }}>
      ...
      <div style={topBar}>
        ...
        <div style={{
          ...
          fontSize: 11, color: L.accent, fontWeight: 600,
      ...
      <div style={{ padding: '24px 28px' }}>
        ...
        <div style={{ ...card, padding: 28, marginBottom: 20 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: L.text, marginBottom: 20 }}>
            Currency Converter
          </div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
              style={{
                ...
                fontSize: 24, fontWeight: 700, fontFamily: L.fontMono,
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 48px 1fr', gap: 12, alignItems: 'center', marginBottom: 24 }}>
            <div>
              ...
              <DropDown value={from} search={fromSearch} setSearch={setFromSearch}
                show={showFrom} setShow={setShowFrom} filtered={filtFrom} onSelect={setFrom} />
            </div>
            <div style={{ paddingTop: 24, textAlign: 'center' }}>
              <button onClick={() => { setFrom(to); setTo(from); }}
                style={{
                  ...
                  background: L.accentSoft, border: `1px solid ${L.accentBorder}`,
                  color: L.accent, fontSize: 18, cursor: 'pointer',
            ...
            <div>
              ...
              <DropDown value={to} search={toSearch} setSearch={setToSearch}
                show={showTo} setShow={setShowTo} filtered={filtTo} onSelect={setTo} />
            </div>
          </div>

          {/* Result */}
          <div style={{
            padding: 22, borderRadius: L.radius,
            background: `linear-gradient(135deg, ${L.accentSoft} 0%, rgba(59,130,246,0.04) 100%)`,
            ...
            <div>
              ...
              <div style={{ fontSize: 36, fontWeight: 800, color: L.accent, fontFamily: L.fontMono, letterSpacing: '-0.02em' }}>
            ...
            <div style={{ fontSize: 48 }}>{toInfo.flag}</div>
          </div>
        </div>

        {/* Quick convert */}
        <div style={{ ...card, padding: 20, marginBottom: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: L.text, marginBottom: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px,1fr))', gap: 10 }}>
            {POPULAR.filter(c => c !== from).map(code => {
              ...
              return (
                <div key={code} onClick={() => setTo(code)} style={{
                  padding: '12px 14px', borderRadius: L.radiusSm, cursor: 'pointer',
                  background: to === code ? L.accentSoft : '#FFFFFF',
                  border: `1px solid ${to === code ? L.accentBorder : L.border}`,
                  transition: 'all 0.15s',
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    <span style={{ fontSize: 16 }}>{info.flag}</span>
                    <span style={{ fontSize: 12, fontWeight: 700, color: to === code ? L.accent : L.text }}>
                      {code}
                    </span>
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: to === code ? L.accent : L.textSub, fontFamily: L.fontMono }}>
        ...
        <div style={{ ...card, overflow: 'hidden' }}>
          <div style={{
            padding: '16px 20px', borderBottom: `1px solid ${L.borderLight}`,
            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: L.text }}>Your Transactions</div>
              <div style={{ fontSize: 12, color: L.textMuted, marginTop: 2 }}>
                All converted to {baseCurrency}
              </div>
            </div>
            <select value={baseCurrency} onChange={e => setBaseCurrency(e.target.value)}
              style={{
                ...
                color: L.text, fontSize: 12, fontFamily: L.font,
          ...
          <div style={{
            padding: '12px 20px', background: L.pageBg,
            borderBottom: `1px solid ${L.borderLight}`,
            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          }}>
            <span style={{ fontSize: 12, color: L.textMuted }}>Total spending in {baseCurrency}</span>
            <span style={{ fontSize: 18, fontWeight: 700, color: L.red, fontFamily: L.fontMono }}>
              {getInfo(baseCurrency).symbol}{totalBase.toLocaleString('en', { maximumFractionDigits: 2 })}
            </span>
          </div>

          <div style={{
            display: 'grid', gridTemplateColumns: '100px 1fr 120px 130px 80px',
            padding: '8px 20px', borderBottom: `1px solid ${L.borderLight}`,
            background: L.pageBg,
          }}>
            {['DATE','VENDOR','ORIGINAL','CONVERTED','CCY'].map(h => (
              <div key={h} style={{ fontSize: 9, fontWeight: 700, color: L.textFaint, letterSpacing: '0.12em' }}>
                {h}
              </div>
            ))}
          </div>
          ...
          {transactions.map((t, i) => {
            ...
            return (
              <div key={t.id} style={{
                display: 'grid', gridTemplateColumns: '100px 1fr 120px 130px 80px',
                padding: '12px 20px',
                borderBottom: i < transactions.length - 1 ? `1px solid ${L.borderLight}` : 'none',
                alignItems: 'center', transition: 'background 0.1s',
              }}
                onMouseEnter={e => e.currentTarget.style.background = L.rowHover}
                onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
              >
                <div style={{ fontSize: 11, color: L.textMuted, fontFamily: L.fontMono }}>
                  {t.txn_date || '—'}
                </div>
                <div style={{ fontSize: 13, fontWeight: 600, color: L.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {t.vendor || '—'}
                </div>
                <div style={{ fontSize: 12, fontWeight: 700, color: L.red, fontFamily: L.fontMono }}>
                  ${Math.abs(t.amount).toLocaleString()}
                </div>
                <div style={{ fontSize: 12, fontWeight: 700, color: L.accent, fontFamily: L.fontMono }}>
                  {bInfo.symbol}{conv.toLocaleString('en', { maximumFractionDigits: 2 })}
                </div>
                <div>
