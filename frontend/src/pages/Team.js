...
const ROLES = {
  admin: {
    ...
    permissions: [
      ...
      'Manage invoices', 'Manage AI actions',
    ],
  },
  accountant: {
    label:       'Accountant',
    ...
    permissions: [
      'View all data', 'Edit transactions',
      'Upload documents', 'Export reports', 'Manage invoices',
    ],
  },
  staff: {
    label:       'Staff',
...
// ── Safe fetch — always returns JSON, never throws HTML ───────
async function safeFetch(url, options = {}) {
  ...
  try {
    ...
    // Backend returned HTML or non-JSON — extract useful message
    throw new Error(
      res.status === 500
        ? 'Server error. Please try again or check the backend logs.'
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRadius: T.radiusLg, width: 480, maxWidth: '90vw',
...
function Btn({ children, variant = 'primary', onClick, disabled, small }) {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      background: disabled ? T.textFaint : s.bg,
      color:      disabled ? T.textMuted  : s.color,
      border: `1px solid ${s.border}`,
      cursor: disabled ? 'not-allowed' : 'pointer',
      fontSize: small ? 11 : 12, fontWeight: 500, fontFamily: T.font,
      transition: 'all 0.15s', opacity: disabled ? 0.6 : 1, whiteSpace: 'nowrap',
    }}>
      {children}
    </button>
  );
...
export default function Team() {
  ...
  const load = async () => {
    try {
      ...
      console.error('load team error:', e.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', background: T.bg, fontFamily: T.font, minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 18, fontWeight: 600, color: T.text, letterSpacing: '-0.02em' }}>Team Access</div>
          <div style={{ fontSize: 12, color: T.textMuted, marginTop: 2 }}>Manage team members and their permissions</div>
        </div>
        <Btn onClick={() => { setModal('invite'); setError(''); }}>
      ...
      <div style={{ padding: '24px 28px' }}>
        ...
        {success && (
          <div style={{ padding: '12px 16px', borderRadius: T.radiusSm, background: T.accentSoft, border: `1px solid ${T.accentBorder}`, color: T.accent, fontSize: 13, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 24 }}>
          {[
            ...
            <div key={sc.label} style={{ background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: T.radius, padding: '18px 20px' }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: T.textMuted, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 10 }}>{sc.label}</div>
              <div style={{ fontSize: 24, fontWeight: 600, color: sc.color, fontFamily: T.fontMono }}>{sc.value}</div>
            </div>
          ))}
        </div>
          ...
          {Object.entries(ROLES).map(([key, role]) => {
            ...
            return (
              <div
                ...
                style={{ background: isSelected ? role.bg : T.bgCard, border: `2px solid ${isSelected ? role.color : T.border}`, borderRadius: T.radius, padding: '16px 18px', cursor: 'pointer', transition: 'all 0.2s', position: 'relative', boxShadow: isSelected ? `0 0 0 3px ${role.bg}` : 'none' }}
              ...
              >
                {isSelected && (
                  <div style={{ position: 'absolute', top: 10, right: 10, width: 20, height: 20, borderRadius: '50%', background: role.color, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Check size={11} color="#fff" />
                  </div>
                )}
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, borderRadius: 8, background: role.bg, border: `1px solid ${role.border}`, color: role.color }}>
                    {role.icon}
                  </span>
                  <span style={{ fontSize: 13, fontWeight: 600, color: isSelected ? role.color : T.text }}>{role.label}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  <ChevronRight size={12} color={isSelected ? role.color : T.textFaint} />
                </div>
                <div style={{ fontSize: 11, color: T.textMuted, lineHeight: 1.5, marginBottom: 10 }}>{role.description}</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {role.permissions.map(p => (
                    <div key={p} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 10, color: isSelected ? T.textSub : T.textMuted }}>
                      <Check size={10} color={role.color} />{p}
                    </div>
                  ))}
                </div>
                <div style={{ marginTop: 10, fontSize: 9, color: isSelected ? role.color : T.textFaint, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                  {isSelected ? '✓ Filtering — click to clear' : 'Click to filter'}
                </div>
              </div>
            );
          })}
        ...
        {/* Filter banner */}
        {activeFilter && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderRadius: T.radiusSm, marginBottom: 16, background: ROLES[activeFilter]?.bg, border: `1px solid ${ROLES[activeFilter]?.border}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 13, color: ROLES[activeFilter]?.color, fontWeight: 500 }}>
                Showing {filteredMembers.length} {ROLES[activeFilter]?.label} member{filteredMembers.length !== 1 ? 's' : ''}
              </span>
            </div>
        ...
        <div style={{ background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: T.radius, overflow: 'hidden' }}>
          <div style={{ padding: '18px 22px', borderBottom: `1px solid ${T.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: T.textMuted, marginTop: 2 }}>
                {filteredMembers.length} member{filteredMembers.length !== 1 ? 's' : ''}{activeFilter ? ` with ${ROLES[activeFilter]?.label} role` : ''}
              </div>
            </div>
            {activeFilter && (
              <Btn small variant="secondary" onClick={() => setModal('invite')}>
                <UserPlus size={11} /> Invite {ROLES[activeFilter]?.label}
              </Btn>
            )}
          </div>

          {filteredMembers.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 140px 120px 120px 200px', padding: '8px 22px', borderBottom: `1px solid ${T.borderSubtle}` }}>
          ...
          {loading && (
            <div style={{ padding: 40, textAlign: 'center', color: T.textMuted }}>Loading team...</div>
          )}
          ...
          {!loading && filteredMembers.length === 0 && (
            <div style={{ textAlign: 'center', padding: 60, color: T.textMuted }}>
              ...
              <div style={{ fontSize: 13, marginBottom: 20 }}>
                {activeFilter ? `Invite someone as ${ROLES[activeFilter]?.label} to get started` : 'Invite your accountant, staff or business partners'}
              </div>
          ...
          {filteredMembers.map((member) => {
            ...
            return (
              <div
                key={member.id}
                style={{ display: 'grid', gridTemplateColumns: '1fr 140px 120px 120px 200px', padding: '14px 22px', borderBottom: `1px solid ${T.borderSubtle}`, alignItems: 'center', transition: 'background 0.1s' }}
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  <button
                    ...
                    style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: T.radiusSm, background: 'transparent', border: `1px solid ${T.border}`, color: T.textMuted, cursor: 'pointer', fontSize: 11, fontFamily: T.font, transition: 'all 0.15s' }}
        ...
        <Modal title="Invite Team Member" onClose={closeModal}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {Object.entries(ROLES).map(([key, role]) => (
                <div key={key} onClick={() => setInvRole(key)} style={{ padding: '12px 14px', borderRadius: T.radiusSm, border: `1px solid ${invRole === key ? role.border : T.border}`, background: invRole === key ? role.bg : 'transparent', cursor: 'pointer', transition: 'all 0.15s' }}>
          ...
          {invEmail && (
            <div style={{ padding: '10px 14px', borderRadius: T.radiusSm, background: T.accentSoft, border: `1px solid ${T.accentBorder}`, fontSize: 12, color: T.textSub, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              Inviting <strong style={{ color: T.text }}>{invEmail}</strong> as{' '}
              <strong style={{ color: ROLES[invRole]?.color }}>{ROLES[invRole]?.label}</strong>
            </div>
          )}
        ...
        <Modal title="Edit Member Role" onClose={closeModal}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {Object.entries(ROLES).map(([key, role]) => (
                <div key={key} onClick={() => setEditRole(key)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: T.radiusSm, border: `1px solid ${editRole === key ? role.border : T.border}`, background: editRole === key ? role.bg : 'transparent', cursor: 'pointer', transition: 'all 0.15s' }}>
                  <span style={{ color: role.color }}>{role.icon}</span>
                  <div style={{ flex: 1 }}>
        ...
        <Modal title="Remove Team Member" onClose={closeModal}>
          <div style={{ textAlign: 'center', padding: '16px 0 24px' }}>
            ...
            <div style={{ fontSize: 13, color: T.textMuted, lineHeight: 1.6 }}>
              This will immediately revoke their access to LedgerFlow. This action cannot be undone.
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <Btn variant="secondary" onClick={closeModal}>Cancel</Btn>
            <button onClick={handleDelete} disabled={saving} style={{ flex: 1, padding: '9px 16px', borderRadius: T.radiusSm, background: T.red, color: '#fff', border: 'none', cursor: saving ? 'not-allowed' : 'pointer', fontSize: 12, fontWeight: 600, fontFamily: T.font, opacity: saving ? 0.7 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
