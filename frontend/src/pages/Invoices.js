...
const saveInvoiceMeta = (meta) => {
  try {
    localStorage.setItem(INVOICE_META_KEY, JSON.stringify(meta));
  } catch {}
};

const buildMetaKey = (invoice) => {
  if (!invoice) return '';
  return invoice.id || invoice.invoice_number || '';
};

const getInvoiceExtras = (invoice) => ({
  terms: invoice?.terms || 'Net 30',
  logo_url: invoice?.logo_url || '',
  from_phone: invoice?.from_phone || '',
  from_bn: invoice?.from_bn || '',
  discount: invoice?.discount ?? 0,
});

const mergeInvoiceWithMeta = (invoice, metaMap) => {
  const key = buildMetaKey(invoice);
  const meta = key ? metaMap[key] || {} : {};
  return {
    ...invoice,
    ...meta,
  };
...
function Field({ label, children, required }) {
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      >
        {label}
        {required && <span style={{ color: L.accent }}> *</span>}
      </div>
      {children}
    </div>
  );
    ...
    >
      ...
      >
        <div>
          ...
          {inv.from_address && (
            <div style={{ fontSize: 13, color: '#444', whiteSpace: 'pre-line', marginBottom: 2 }}>
              {inv.from_address}
            </div>
          )}
          {inv.from_email && (
      ...
      >
        <div>
          ...
          {inv.to_address ? (
            <div style={{ fontSize: 13, color: '#444', whiteSpace: 'pre-line' }}>
              {inv.to_address}
            </div>
          ) : null}
        </div>
        ...
        <div>
          ...
          {[
            ...
            { label: 'Terms:', value: inv.terms || 'Net 30' },
            ...
            >
              <span style={{ minWidth: 100 }}>{row.label}</span>
              <span>{row.value}</span>
            </div>
          ))}
        </div>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        <thead>
          <tr>
            {[
              { label: '#', align: 'left', w: 30 },
              { label: 'Product or service', align: 'left', w: 160 },
              { label: 'Description', align: 'left', w: '' },
              { label: 'Qty', align: 'right', w: 50 },
              { label: 'Rate', align: 'right', w: 90 },
              { label: 'Amount', align: 'right', w: 90 },
            ].map((col) => (
              <th
                key={col.label}
                style={{
            ...
            lineItems.map((item, i) => {
              ...
              return (
                <tr key={i} style={{ borderBottom: '1px solid #eee' }}>
                  <td style={{ padding: '14px 8px', fontSize: 13, color: '#1a1a2e', verticalAlign: 'top' }}>
                    {i + 1}.
                  </td>
                  <td style={{ padding: '14px 8px', fontSize: 13, fontWeight: 700, color: '#1a1a2e', verticalAlign: 'top' }}>
                    {item.service || item.description || '—'}
                  </td>
                  <td style={{ padding: '14px 8px', fontSize: 13, color: '#444', verticalAlign: 'top' }}>
                    {item.service ? item.description : ''}
                  </td>
                  <td style={{ padding: '14px 8px', fontSize: 13, color: '#1a1a2e', textAlign: 'right', verticalAlign: 'top' }}>
                    {qty}
                  </td>
                  <td style={{ padding: '14px 8px', fontSize: 13, color: '#1a1a2e', textAlign: 'right', verticalAlign: 'top' }}>
                    ${price.toFixed(2)}
                  </td>
                  <td style={{ padding: '14px 8px', fontSize: 13, fontWeight: 600, color: '#1a1a2e', textAlign: 'right', verticalAlign: 'top' }}>
                    ${amount.toFixed(2)}
                  </td>
                </tr>
              );
        ...
        <div style={{ width: 300 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #ddd' }}>
            ...
            <span style={{ fontSize: 13, color: '#444' }}>
              {paid ? `-$${totalAmt.toFixed(2)}` : '$0.00'}
            </span>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0' }}>
            <span style={{ fontSize: 14, color: '#1a1a2e', fontWeight: 700 }}>Balance due</span>
            <span style={{ fontSize: 14, color: paid ? '#52b788' : '#1a1a2e', fontWeight: 700 }}>
              {paid ? '$0.00' : `$${totalAmt.toFixed(2)}`}
            </span>
          </div>

          {paid && (
            <div style={{ textAlign: 'right', color: '#52b788', fontSize: 13, marginTop: 4 }}>
              Paid in Full
            </div>
          )}
        </div>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 28px', gap: 8, marginBottom: 8 }}>
      ...
      <button
        ...
        onMouseEnter={(e) => {
          e.currentTarget.style.borderColor = L.redBorder;
          e.currentTarget.style.color = L.red;
        }}
        onMouseLeave={(e) => {
          e.currentTarget.style.borderColor = L.border;
          e.currentTarget.style.color = L.textMuted;
        }}
      >
        <X size={12} />
      </button>
...
export default function Invoices() {
  ...
  const mergeInvoices = useCallback((serverInvoices) => {
    const metaMap = loadInvoiceMeta();
    return (Array.isArray(serverInvoices) ? serverInvoices : []).map((inv) =>
      mergeInvoiceWithMeta(inv, metaMap)
    );
  }, []);

  const persistInvoiceMeta = useCallback((invoiceLike) => {
    const key = buildMetaKey(invoiceLike);
    if (!key) return;

    const metaMap = loadInvoiceMeta();
    metaMap[key] = getInvoiceExtras(invoiceLike);
    saveInvoiceMeta(metaMap);
  }, []);

  const removeInvoiceMeta = useCallback((invoiceLike) => {
    const key = buildMetaKey(invoiceLike);
    if (!key) return;
  ...
  const handleItemAdd = useCallback(() => {
    setFormState((p) => ({ ...p, items: [...p.items, emptyItem()] }));
  }, []);
  ...
  const { subtotal, discount: discAmt, tax, total } = calcTotals(
    form.items,
    form.tax_rate,
    form.discount
  );
  ...
  const openEdit = (inv) => {
    ...
    const li = (inv.line_items || inv.items || []).map((it) => ({
    ...
    setFormState({
      ...
      date: inv.date || todayString(),
      due_date: inv.due_date || plus30Days(),
  ...
  const handleSave = async (isEdit) => {
    ...
    try {
      ...
      if (!res.ok) {
        window.alert(data?.detail || 'Failed to save invoice.');
        return;
      }

      const mergedSaved = {
        ...(data || {}),
        ...getInvoiceExtras(payload),
  ...
  const handleDelete = async (id) => {
    ...
    if (res.ok || res.status === 204) {
      if (invoiceToDelete) {
        removeInvoiceMeta(invoiceToDelete);
      }
      setModal(null);
      await load();
    }
  ...
  const exportPDF = (inv) => {
    try {
      ...
      let y = 40;
      if (inv.from_bn) {
        doc.text(`BN ${inv.from_bn}`, 14, y);
        y += 5;
      }
      if (inv.from_address) {
        doc.text(inv.from_address, 14, y);
        y += 5;
      }
      if (inv.from_email) {
        doc.text(inv.from_email, 14, y);
        y += 5;
      }
      if (inv.from_phone) {
        doc.text(inv.from_phone, 14, y);
      }

      if (inv.logo_url) {
        try {
          doc.addImage(inv.logo_url, 'JPEG', 150, 10, 46, 24);
        } catch {
          try {
            doc.addImage(inv.logo_url, 'PNG', 150, 10, 46, 24);
          } catch {}
        }
      }
      ...
      doc.setFontSize(11);
      doc.setFont('helvetica', 'normal');
      doc.text(inv.to_name || '—', 14, boxY + 16);

      if (inv.to_email) {
        doc.setFontSize(9);
        doc.text(inv.to_email, 14, boxY + 22);
      }

      if (inv.to_address) {
        doc.setFontSize(9);
        doc.text(inv.to_address, 14, boxY + 27);
      }

      doc.setFontSize(10);
      doc.setFont('helvetica', 'bold');
      doc.text('Invoice details', 110, boxY + 8);
      ...
      [
        ...
        doc.text(label, 110, boxY + 16 + i * 6);
        doc.text(value, 150, boxY + 16 + i * 6);
      });
      ...
      autoTable(doc, {
        ...
        body: rows.length ? rows : [['1.', 'No items', '', '', '', '']],
        styles: {
          fontSize: 9,
      ...
      const addRow = (label, value, bold) => {
        ...
        doc.line(totalsX, ty + 2, 196, ty + 2);
        ty += 9;
      };
      ...
      doc.text(paid ? '$0.00' : `$${tot.toFixed(2)}`, 196, ty + 2, { align: 'right' });

      if (paid) {
        ty += 10;
        doc.setFontSize(12);
        doc.setTextColor(82, 183, 136);
        doc.text('Paid in Full', 196, ty, { align: 'right' });
      }

      if (inv.notes) {
        const ny = tableEnd + 60;
        doc.setDrawColor(226, 232, 240);
        doc.line(14, ny, 196, ny);
        doc.setFontSize(8);
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(148, 163, 184);
        doc.text('NOTES', 14, ny + 6);
        doc.setFontSize(9);
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(100, 116, 139);
        doc.text(doc.splitTextToSize(inv.notes, 170), 14, ny + 13);
      }
  ...
  const totalDue = invoices
    .filter((i) => ['due', 'overdue'].includes(i.status))
    .reduce((s, i) => s + (Number(i.total) || 0), 0);
  ...
  const renderForm = (isEdit) => (
    <>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
        <Field label="Invoice #">
          <input
            style={{ ...inp, background: '#f7fafc', color: L.textMuted }}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
        <div style={{ padding: 16, background: L.pageBg, borderRadius: 10, border: `1px solid ${L.border}` }}>
          <div
            style={{
              fontSize: 10,
              fontWeight: 700,
          ...
          <input
            ...
            onChange={(e) => sf('from_name', e.target.value)}
          />
          <input
            style={{ ...inp, marginBottom: 8 }}
            placeholder="Email"
            value={form.from_email}
            onChange={(e) => sf('from_email', e.target.value)}
          />
          <input
            style={{ ...inp, marginBottom: 8 }}
            placeholder="Address"
            value={form.from_address}
            onChange={(e) => sf('from_address', e.target.value)}
          />
          <input
            style={{ ...inp, marginBottom: 8 }}
            placeholder="Phone (optional)"
            value={form.from_phone}
            onChange={(e) => sf('from_phone', e.target.value)}
          />
          <input
            style={{ ...inp, marginBottom: 14 }}
            placeholder="Business Number / BN (optional)"
            value={form.from_bn}
            onChange={(e) => sf('from_bn', e.target.value)}
          />
          ...
          <input
            ...
            style={{ fontSize: 12, color: L.textMuted, width: '100%' }}
          />
        ...
        <div style={{ padding: 16, background: L.pageBg, borderRadius: 10, border: `1px solid ${L.border}` }}>
          ...
          <input
            ...
            value={form.to_name}
            onChange={(e) => sf('to_name', e.target.value)}
          />
          <input
            style={{ ...inp, marginBottom: 8 }}
            placeholder="Client email"
            value={form.to_email}
            onChange={(e) => sf('to_email', e.target.value)}
          />
          <input
            style={inp}
            placeholder="Client address (optional)"
            value={form.to_address}
            onChange={(e) => sf('to_address', e.target.value)}
          />
        </div>
      </div>

      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: L.text, marginBottom: 10 }}>
          Line Items
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
        <Field label="Tax Rate (%)">
          <input
            ...
            placeholder="0"
            value={form.tax_rate}
            onChange={(e) => sf('tax_rate', e.target.value)}
          />
        </Field>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 20 }}>
        ...
        >
          {[
            ...
            { label: `Discount (${form.discount}%)`, value: `−$${discAmt.toFixed(2)}` },
            { label: `Tax (${form.tax_rate}%)`, value: `$${tax.toFixed(2)}` },
          ].map((row) => (
            <div
              key={row.label}
              style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0 0' }}>
            ...
            <span style={{ fontSize: 18, fontWeight: 800, color: L.accent }}>
              ${total.toFixed(2)}
            </span>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 20 }}>
        ...
        <button
          ...
          style={{
            ...
            padding: '9px 22px',
            borderRadius: L.radiusSm,
            background: saving ? L.textFaint : L.accent,
            color: '#fff',
            border: 'none',
  ...
  return (
    <div style={page}>
        ...
        <div style={{ ...card, overflow: 'hidden' }}>
          ...
          {invoices.length > 0 && (
            <div
              style={{
                display: 'grid',
                gridTemplateColumns: '130px 1fr 1fr 110px 120px 1fr',
                padding: '8px 20px',
                background: L.pageBg,
                borderBottom: `1px solid ${L.border}`,
              }}
            >
              {['NUMBER', 'CLIENT', 'FROM', 'TOTAL', 'STATUS', 'ACTIONS'].map((h) => (
                <div
                  key={h}
                  style={{ fontSize: 9, fontWeight: 700, color: L.textFaint, letterSpacing: '0.12em' }}
                >
                  {h}
                </div>
              ))}
            ...
            return (
              <div
                ...
                style={{
                  ...
                  padding: '14px 20px',
                  alignItems: 'center',
                  borderBottom: i < invoices.length - 1 ? `1px solid ${L.border}` : 'none',
              ...
              >
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: L.text }}>
                    {inv.to_name || '—'}
                  </div>
                  {inv.to_email && (
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: inv.status === 'paid' ? L.accent : L.text }}>
                ...
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                  <button
                    ...
                    style={{
                      ...
                      padding: '5px 9px',
                      borderRadius: L.radiusSm,
                      background: L.blueSoft,
                      border: `1px solid ${L.blueBorder}`,
                      color: L.blue,
                      cursor: 'pointer',
                      fontSize: 11,
                      fontWeight: 600,
                  ...
                  <button
                    ...
                    onMouseEnter={(e) => {
                      e.currentTarget.style.borderColor = L.accentBorder;
      ...
      {modal === 'edit' && selected && (
        <Modal title={`Edit — ${selected.invoice_number}`} onClose={() => setModal(null)} wide>
          {renderForm(true)}
        </Modal>
      )}
        ...
        <Modal title={`Invoice ${selected.invoice_number}`} onClose={() => setModal(null)} wide>
          <div style={{ display: 'flex', gap: 8, marginBottom: 20, justifyContent: 'flex-end' }}>
            ...
            {selected.status !== 'paid' && (
              <button
                onClick={() => handleStatus(selected.id, 'paid')}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: 6,
