...
import {
  ...
  CheckCircle, RefreshCw, MessageCircle, ScanLine,
} from 'lucide-react';
...
export default function SmartSearch() {
  ...
  const handleSearch = async () => {
    ...
    // Normalise common shorthand queries
    const normalized = q
      .replace(/\boverdue\b/gi, 'overdue payment_status:overdue')
      .replace(/\bunpaid\b/gi,  'unpaid payment_status:due OR payment_status:overdue')
      .replace(/\bpaid\b/gi,    'paid payment_status:paid');
    ...
    try {
      ...
      // Client-side filter for overdue/paid/due when backend misses them
      const ql = q.toLowerCase();
      if (ql.includes('overdue')) {
        hits = hits.filter(r => (r.metadata?.payment_status || '').toLowerCase() === 'overdue');
      } else if (ql.includes('unpaid')) {
        hits = hits.filter(r => ['due','overdue','partially_paid'].includes((r.metadata?.payment_status || '').toLowerCase()));
      } else if (ql === 'paid') {
        hits = hits.filter(r => (r.metadata?.payment_status || '').toLowerCase() === 'paid');
      }

      setResults(hits);
      setSearched(true);
      if (!hits.length) setError(`No documents found for "${q}".`);
    } catch (e) {
            ...
            <div style={{ ...card, padding:isMobile?'16px':'24px', marginBottom:20 }}>
              <div style={{ display:'flex', gap:10, flexDirection:isMobile?'column':'row' }}>
                <div style={{ flex:1, display:'flex', alignItems:'center', gap:10, padding:'12px 16px', borderRadius:L.radiusSm, background:L.pageBg, border:`2px solid ${L.border}`, transition:'border-color 0.2s' }}
                  ...
                  <input
                    value={query}
                    onChange={e => { setQuery(e.target.value); if (searched) { setResults([]); setSearched(false); setError(''); } }}
                  ...
                  />
                  {query && (
                    <button onClick={() => { setQuery(''); setResults([]); setError(''); setSearched(false); }}
                      style={{ background:'none', border:'none', cursor:'pointer', color:L.textMuted, fontSize:16, lineHeight:1 }}>×</button>
                  )}
                </div>
              ...
              <div style={{ marginTop:14 }}>
                ...
                <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                  {quickSearches.map(({ label, q }) => (
                    <button key={q}
                      onClick={() => { setQuery(q); setResults([]); setSearched(false); setError(''); }}
                      style={{ padding:'5px 12px', borderRadius:20, cursor:'pointer', fontSize:11, fontWeight:500, border:`1px solid ${L.border}`, background:'#fff', color:L.textSub, fontFamily:L.font, transition:'all 0.15s' }}
                      onMouseEnter={e => { e.currentTarget.style.borderColor=ACCENT; e.currentTarget.style.color=ACCENT; e.currentTarget.style.background='rgba(10,185,138,0.04)'; }}
            ...
            {results.length > 0 && (
              <div style={{ ...card, overflow:'hidden' }}>
                <div style={{ padding:isMobile?'14px 16px':'16px 22px', borderBottom:`1px solid ${L.border}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  ...
                  <div style={{ fontSize:12, color:L.textMuted }}>{results.length} document{results.length !== 1 ? 's' : ''} found</div>
                </div>
                {results.map((r, i) => {
                    ...
                    <div key={i}
                      ...
                      <div style={{ display:'flex', gap:6, flexShrink:0, flexDirection:isMobile?'column':'row' }}>
                        <button
                          onClick={async () => {
                            try {
                              const res  = await fetch(`${BASE}/documents/${r.document_id}/view`, { headers:{ Authorization:`Bearer ${getToken()}` } });
                        ...
                        <button
                          onClick={() => askAndOpen(`Tell me about this document: ${meta.filename} — vendor: ${meta.vendor}, amount: ${meta.amount}, date: ${meta.doc_date}, status: ${meta.payment_status}`)}
            ...
            {!searching && !error && !searched && (
              <div style={{ ...card, padding:isMobile?'40px 20px':60, textAlign:'center' }}>
                ...
                <div style={{ fontSize:13, color:L.textMuted, maxWidth:400, margin:'0 auto', lineHeight:1.65 }}>
                  Search by vendor name, date, amount, or payment status. Try "overdue", "paid", or "last month".
                </div>
              </div>
            )}
          </>
            ...
            {duplicates.length === 0 ? (
              ...
              duplicates.map((d, i) => (
                <div key={i} style={{ padding:isMobile?'14px 16px':'16px 22px', borderBottom:i<duplicates.length-1?`1px solid ${L.border}`:'none', background:'rgba(245,158,11,0.04)' }}>
                  ...
                  <button
                    onClick={() => askAndOpen(`Are these two documents duplicates? "${d.doc1_content}" and "${d.doc2_content}"`)}
