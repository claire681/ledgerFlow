  ...
  email: {
    ...
    fields: [
      ...
      },
      {
        ...
        secret:      true,
        required:    true,
        helper:      'Use a Google App Password, not your normal Gmail password.',
      },
      {
        key:         'from_name',
        label:       'From Name',
        placeholder: 'LedgerFlow',
        secret:      false,
        required:    false,
        helper:      null,
      },
...
function SetupModal({ provider, onClose, onSave }) {
  ...
  const handleSave = async () => {
    ...
    const missing = form.fields
      .filter(f => f.required && f.type !== 'toggle' && !String(values[f.key] || '').trim())
      .map(f => f.label);
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, backdropFilter: 'blur(4px)' }}>
      <div style={{ background: '#fff', borderRadius: 16, width: 480, maxWidth: '92vw', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 60px rgba(0,0,0,0.18)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px', borderBottom: `1px solid ${L.border}` }}>
          ...
          <button onClick={onClose} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: L.textMuted, display: 'flex' }}>
            <X size={18} />
          </button>
        </div>
          ...
          {isOAuth && (
            <div style={{ padding: 16, borderRadius: L.radiusSm, background: 'rgba(14,165,233,0.08)', border: '1px solid rgba(14,165,233,0.2)' }}>
              ...
              <div style={{ fontSize: 13, color: L.textMuted, lineHeight: 1.6 }}>
                This integration requires OAuth authorization which is coming soon. We are working on a secure connection flow for this provider.
              </div>
            </div>
          )}
                ...
                <div key={field.key}>
                  ...
                  {field.type === 'toggle' ? (
                    <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
                      ...
                      <span style={{ fontSize: 13, color: L.textSub }}>
                        {values[field.key] ? 'Enabled (Test Mode)' : 'Disabled (Live Mode)'}
                      </span>
                    </label>
                  ) : (
                    /* Text / password input */
                    <div style={{ position: 'relative' }}>
                      <input
                        type={field.secret && !showPass[field.key] ? 'password' : 'text'}
...
function IntegrationCard({ integration, onConnect, onDisconnect, onSync, onTest, loading, testResult }) {
  ...
  return (
    <div
      style={{ ...card, padding: 24, display: 'flex', flexDirection: 'column', gap: 14, transition: 'box-shadow 0.2s' }}
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: `${meta.color}15`, border: `1px solid ${meta.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <Icon size={22} color={meta.color} />
          </div>
          <div>
            <div style={{ fontSize: 14, fontWeight: 700, color: L.text }}>{integration.name}</div>
            <div style={{ fontSize: 11, color: L.textMuted, marginTop: 2, lineHeight: 1.4 }}>{integration.description}</div>
          </div>
        </div>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 20, fontSize: 10, fontWeight: 700, color: ss.color, background: ss.bg, border: `1px solid ${ss.border}`, flexShrink: 0 }}>
        ...
        {!isConnected ? (
          ...
          <>
            <button
              ...
              disabled={isBusy}
              style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '9px', borderRadius: L.radiusSm, background: 'rgba(10,185,138,0.08)', border: '1px solid rgba(10,185,138,0.2)', color: ACCENT, cursor: isBusy ? 'not-allowed' : 'pointer', fontSize: 12, fontWeight: 600, fontFamily: L.font, transition: 'all 0.15s' }}
            ...
            {isTestable && (
              <button
                ...
                disabled={isBusy}
                style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '9px 12px', borderRadius: L.radiusSm, background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.2)', color: '#8B5CF6', cursor: isBusy ? 'not-allowed' : 'pointer', fontSize: 12, fontFamily: L.font, transition: 'all 0.15s', fontWeight: 600 }}
            ...
            <button
              ...
              disabled={isBusy}
              style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '9px 12px', borderRadius: L.radiusSm, background: 'transparent', border: `1px solid ${L.border}`, color: L.textMuted, cursor: isBusy ? 'not-allowed' : 'pointer', fontSize: 12, fontFamily: L.font, transition: 'all 0.15s' }}
...
export default function Integrations() {
  ...
  const handleSaveConfig = async (provider, config) => {
    ...
    try {
      const data = await apiFetch(`/integrations/${provider}/connect`, {
        method: 'POST',
        body:   JSON.stringify({ config }),
      });
      showSuccess(data.message || `${provider} connected`);
      await load();
    } finally {
  ...
  const handleDisconnect = async (provider) => {
    ...
    setError('');
    try {
      const data = await apiFetch(`/integrations/${provider}/disconnect`, { method: 'POST' });
      showSuccess(data.message || `${provider} disconnected`);
      await load();
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  const handleSync = async (provider) => {
    ...
    setError('');
    try {
      const data = await apiFetch(`/integrations/${provider}/sync`, { method: 'POST' });
      showSuccess(data.message || `${provider} synced`);
      await load();
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  const handleTest = async (provider) => {
    ...
    setError('');
    try {
      const data = await apiFetch(`/integrations/${provider}/test`, { method: 'POST' });
      setTestResult({ provider, success: data.success, message: data.message });
      if (data.success) showSuccess(data.message);
    } catch (e) {
      setTestResult({ provider, success: false, message: e.message });
    } finally {
      setActionLoading(null);
    }
  };
  ...
  return (
    <div style={page}>
      ...
      <div style={topBar}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 700, color: L.text, letterSpacing: '-0.02em' }}>Integrations</div>
      ...
      <div style={{ padding: '24px 28px' }}>
        ...
        {success && (
          <div style={{ padding: '12px 16px', borderRadius: L.radiusSm, background: 'rgba(10,185,138,0.08)', border: '1px solid rgba(10,185,138,0.2)', color: ACCENT, fontSize: 13, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}>
            <CheckCircle size={14} /> {success}
          </div>
        )}
        {error && (
          <div style={{ padding: '12px 16px', borderRadius: L.radiusSm, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', color: '#EF4444', fontSize: 13, marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 24 }}>
            {[
              ...
              <div key={s.label} style={{ ...card, padding: '16px 20px' }}>
                <div style={{ fontSize: 9, fontWeight: 700, color: L.textFaint, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 8 }}>{s.label}</div>
