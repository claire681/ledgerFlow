...
function BriefingSettingsModal({ settings, onClose, onSave }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, backdropFilter: 'blur(4px)' }}>
      <div style={{ background: '#fff', borderRadius: 16, width: 440, maxWidth: '92vw', boxShadow: '0 24px 60px rgba(0,0,0,0.18)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={onClose} style={{ flex: 1, padding: '10px', borderRadius: L.radiusSm, background: 'transparent', border: `1px solid ${L.border}`, color: L.textMuted, cursor: 'pointer', fontSize: 13, fontFamily: L.font }}>
              Cancel
            </button>
...
function BriefingCard({ onAskWithData }) {
  ...
  const loadSettings = async () => {
    try {
      ...
      console.error('briefing settings error:', e);
    }
  };
  ...
  const handleSendNow = async () => {
    ...
    try {
      const res  = await fetch(`${BASE}/briefing/send`, {
        method:  'POST',
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      const data = await res.json();
  ...
  return (
    <>
      <div style={{ ...card, padding: '20px 24px', marginBottom: 20, border: `1px solid rgba(10,185,138,0.2)`, background: 'linear-gradient(135deg, rgba(10,185,138,0.04) 0%, rgba(14,165,233,0.04) 100%)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: L.textMuted, marginTop: 2 }}>
                AI sends a daily financial summary to your email
              </div>
            </div>
          </div>
          <button
            onClick={() => setShowSettings(true)}
            style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: L.radiusSm, background: 'transparent', border: `1px solid ${L.border}`, color: L.textMuted, cursor: 'pointer', fontSize: 11, fontFamily: L.font, transition: 'all 0.15s' }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(10,185,138,0.3)'; e.currentTarget.style.color = ACCENT; }}
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
            ...
            {bSettings.briefing_enabled && (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 20, background: 'rgba(14,165,233,0.08)', border: '1px solid rgba(14,165,233,0.2)', fontSize: 11, color: '#0EA5E9' }}>
                <Clock size={10} /> Sends at {bSettings.briefing_time} · {bSettings.briefing_timezone?.replace('_', ' ')}
              </div>
            )}
            {bSettings.last_briefing_at && (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 20, background: L.pageBg, border: `1px solid ${L.border}`, fontSize: 11, color: L.textMuted }}>
                <CheckCircle size={10} color={ACCENT} /> Last sent: {formatTime(bSettings.last_briefing_at)}
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px,1fr))', gap: 8, marginBottom: 16 }}>
          {[
            ...
            { emoji: '📋', label: 'Documents needing review' },
          ].map(item => (
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button
            ...
            disabled={sending}
            style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '10px', borderRadius: L.radiusSm, background: sending ? L.textFaint : ACCENT, color: '#fff', border: 'none', cursor: sending ? 'not-allowed' : 'pointer', fontSize: 13, fontWeight: 600, fontFamily: L.font, boxShadow: sending ? 'none' : '0 4px 12px rgba(10,185,138,0.3)', transition: 'all 0.15s' }}
          >
            <Mail size={13} />
            {sending ? 'Sending briefing...' : 'Send Me My Briefing Now'}
          </button>
          <button
            onClick={() => onAskWithData('Give me a complete financial briefing. What problems do you see and what should I do today?')}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 16px', borderRadius: L.radiusSm, background: 'rgba(10,185,138,0.08)', border: '1px solid rgba(10,185,138,0.2)', color: ACCENT, cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: L.font, transition: 'all 0.15s' }}
      ...
      {showSettings && (
        <BriefingSettingsModal
          ...
          onSave={(data) => { setBSettings(prev => ({ ...prev, ...data })); }}
        />
      )}
    </>
  );
  ...
  return (
    <div style={{ padding:'36px 20px', textAlign:'center', display:'flex', flexDirection:'column', alignItems:'center', gap:10 }}>
      ...
      <div style={{ display:'flex', gap:8, marginTop:4 }}>
        {action && onAction && (
          <button onClick={onAction} style={{ padding:'7px 16px', borderRadius:L.radiusSm, background:L.accent, color:'#FFFFFF', border:'none', cursor:'pointer', fontSize:12, fontWeight:600, fontFamily:L.font }}>
            {action}
          </button>
        )}
        {onAsk && (
...
function MetricCard({ label, value, sub, color, icon: Icon, loading, onClick, isEmpty, emptyText }) {
  return (
    <div
      onClick={onClick}
      style={{ ...card, padding:'20px 22px', cursor:onClick ? 'pointer' : 'default', transition:'all 0.2s', position:'relative', overflow:'hidden' }}
      onMouseEnter={e => { if (onClick) { e.currentTarget.style.borderColor = L.accentBorder; e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = L.shadowMd; } }}
      onMouseLeave={e => { e.currentTarget.style.borderColor = L.border; e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = L.shadow; }}
    >
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div style={{ flex:1 }}>
          <div style={{ fontSize:9, fontWeight:700, color:L.textMuted, letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:10 }}>{label}</div>
          {loading ? (
            <div style={{ height:28, width:80, borderRadius:6, background:L.pageBg, animation:'pulse 1.5s infinite' }}/>
          ) : isEmpty ? (
            <div>
              <div style={{ fontSize:20, fontWeight:700, color:L.textFaint, fontFamily:L.fontMono }}>$0</div>
              <div style={{ fontSize:10, color:L.accent, marginTop:6, fontWeight:500 }}>{emptyText}</div>
            </div>
          ) : (
            <div style={{ fontSize:26, fontWeight:700, color, letterSpacing:'-0.02em', lineHeight:1, fontFamily:L.fontMono }}>{value}</div>
          )}
          <div style={{ fontSize:11, color:L.textMuted, marginTop:6 }}>{sub}</div>
        </div>
      ...
      {onClick && (
        <div style={{ fontSize:10, color:L.accent, marginTop:12, display:'flex', alignItems:'center', gap:4, borderTop:`1px solid ${L.pageBg}`, paddingTop:10 }}>
          <Sparkles size={9}/>Click to ask AI
        </div>
      )}
    </div>
  );
...
export default function Dashboard() {
  ...
  return (
    <div style={page}>
      ...
      <div style={topBar}>
        <div>
          ...
          <div style={{ fontSize:12, color:L.textMuted, marginTop:3 }}>
            {hasData ? 'Here is your business financial summary' : 'Welcome to LedgerFlow — let us get your finances set up'}
          </div>
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={load} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 14px', borderRadius:L.radiusSm, background:'transparent', border:`1px solid ${L.border}`, color:L.textMuted, cursor:'pointer', fontSize:12, fontFamily:L.font }}
            onMouseEnter={e => e.currentTarget.style.borderColor = L.accentBorder}
            onMouseLeave={e => e.currentTarget.style.borderColor = L.border}>
            <RefreshCw size={13}/> Refresh
          </button>
          <button
            onClick={async () => {
              try { const t = await getTransactions({}); if (!t.data || t.data.length === 0) { alert('No transactions to export yet.'); return; } generateReport(stats, t.data, companyName || 'LedgerFlow'); }
          ...
          <button
            onClick={() => askWithData('Give me a complete summary of my business finances with revenue, expenses, net profit, and 3 specific recommendations.')}
      ...
      {success && (
        <div style={{ margin:'0 28px 16px', padding:'12px 16px', borderRadius:L.radiusSm, background:L.accentSoft, border:`1px solid ${L.accentBorder}`, color:L.accent, fontSize:13, fontWeight:500, display:'flex', alignItems:'center', gap:8 }}>
      ...
      <div style={{ padding:'24px 28px', maxWidth:1400, margin:'0 auto' }}>
        ...
        {/* Money Brain Daily Briefing (existing) */}
        {briefing && (briefing.observations?.length > 0 || briefing.actions?.length > 0) && (
          <div style={{ ...card, padding:'20px 24px', marginBottom:20, border:`1px solid rgba(10,185,138,0.2)`, background:'linear-gradient(135deg, rgba(10,185,138,0.04) 0%, rgba(14,165,233,0.04) 100%)' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                <div style={{ width:36, height:36, borderRadius:10, background:'linear-gradient(135deg, #0AB98A 0%, #0EA5E9 100%)', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 2px 8px rgba(10,185,138,0.3)' }}>
                  <Brain size={18} color="#FFFFFF"/>
                </div>
                <div>
                  <div style={{ fontSize:14, fontWeight:700, color:L.text }}>LedgerFlow Money Brain</div>
                  <div style={{ fontSize:11, color:L.textMuted }}>Proactive financial intelligence — updated now</div>
                </div>
              </div>
              <button onClick={() => askWithData('Give me a full financial briefing. What problems do you see and what should I do today?')}
                ...
                <Sparkles size={10}/> Full briefing
              </button>
            </div>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                  {briefing.observations.map((obs, i) => (
                    <div key={i} onClick={() => askWithData(`Tell me more about this: ${obs}`)}
                      style={{ display:'flex', alignItems:'flex-start', gap:8, padding:'8px 12px', borderRadius:L.radiusSm, background:'rgba(10,185,138,0.06)', border:'1px solid rgba(10,185,138,0.12)', cursor:'pointer', transition:'all 0.15s' }}
                      onMouseEnter={e => e.currentTarget.style.background = 'rgba(10,185,138,0.1)'}
                      onMouseLeave={e => e.currentTarget.style.background = 'rgba(10,185,138,0.06)'}>
                ...
                <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                  {briefing.actions.map((action, i) => (
                    <button key={i} onClick={() => askWithData(`Help me: ${action}`)}
                      style={{ padding:'6px 14px', borderRadius:20, background:L.accent, color:'#FFFFFF', border:'none', cursor:'pointer', fontSize:11, fontWeight:600, fontFamily:L.font, display:'flex', alignItems:'center', gap:5, transition:'all 0.15s', boxShadow:'0 2px 6px rgba(10,185,138,0.3)' }}
        ...
        {!loading && !hasData && (
          <div style={{ ...card, padding:'20px 24px', marginBottom:20, background:'linear-gradient(135deg, rgba(10,185,138,0.06) 0%, rgba(14,165,233,0.06) 100%)', border:`1px solid ${L.accentBorder}` }}>
            <div style={{ fontSize:15, fontWeight:700, color:L.text, marginBottom:6 }}>🚀 Get started with LedgerFlow</div>
            <div style={{ fontSize:13, color:L.textMuted, marginBottom:16, lineHeight:1.6 }}>Your dashboard is ready. Upload your first document to start tracking expenses, revenue, and taxes automatically.</div>
            <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
              {[
                { icon:Upload,     label:'Upload a document', sub:'Invoice or receipt',    path:'/documents', color:L.accent   },
                { icon:Receipt,    label:'Scan a receipt',    sub:'AI reads it instantly', path:'/receipts',  color:L.blue     },
                { icon:TrendingUp, label:'Set a budget',      sub:'Track spending limits', path:'/budgets',   color:'#8B5CF6'  },
                { icon:Sparkles,   label:'Ask AI anything',   sub:'Your financial advisor', onClick:() => askAndOpen('I just joined LedgerFlow. What should I do first?'), color:'#F59E0B' },
              ].map(item => (
                <button key={item.label} onClick={item.onClick || (() => window.location.href = item.path)}
                  style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 16px', borderRadius:L.radiusSm, background:'#FFFFFF', border:`1px solid ${L.border}`, cursor:'pointer', fontFamily:L.font, transition:'all 0.15s' }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = item.color; e.currentTarget.style.background = `${item.color}08`; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = L.border; e.currentTarget.style.background = '#FFFFFF'; }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14, marginBottom:20 }}>
          <MetricCard label="Documents" value={stats?.docs_processed ?? 0} sub="Total uploaded" color={L.accent} icon={FileText} loading={loading} isEmpty={!loading && !stats?.docs_processed} emptyText="Upload your first document →" onClick={() => askWithData('How many documents have I uploaded and what was extracted?')} />
          <MetricCard label="Total Expenses" value={fmt(stats?.total_expenses)} sub="This period" color={L.red} icon={ArrowDown} loading={loading} isEmpty={!loading && !stats?.total_expenses} emptyText="No expenses recorded yet" onClick={() => askWithData('Break down my total expenses by category. Which costs the most?')} />
          <MetricCard label="Revenue" value={fmt(stats?.total_revenue)} sub="This period" color={L.accent} icon={ArrowUp} loading={loading} isEmpty={!loading && !stats?.total_revenue} emptyText="No revenue recorded yet" onClick={() => askWithData('Summarize my total revenue. Where is it coming from?')} />
          <MetricCard label="Net Position" value={fmt(net)} sub="Revenue minus expenses" color={net >= 0 ? L.accent : L.red} icon={net >= 0 ? ArrowUp : ArrowDown} loading={loading} isEmpty={!loading && !hasData} emptyText="Add data to see net position" onClick={() => askWithData('What is my net financial position and how can I improve it?')} />
        </div>
          ...
          <div style={{ marginBottom:20 }}>
            <div style={{ fontSize:13, fontWeight:700, color:L.text, marginBottom:10, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <div style={{ display:'flex', alignItems:'center', gap:7 }}>
                ...
                AI Insights
                {companyName && <span style={{ color:L.textMuted, fontWeight:400 }}>— for {companyName}</span>}
              </div>
              ...
              {insights.map((ins, i) => {
                ...
                return (
                  <div key={i} onClick={() => askWithData(ins.action || ins.title)}
                    style={{ padding:'12px 14px', borderRadius:L.radiusSm, background:s.bg, border:`1px solid ${s.border}`, cursor:'pointer', transition:'all 0.15s' }}
                    onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-1px)'}
                    onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:14, marginBottom:20 }}>
          <div style={{ ...card, padding:'22px 24px', position:'relative' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16 }}>
              <div>
                <div style={{ fontSize:14, fontWeight:700, color:L.text }}>Cash Flow</div>
                <div style={{ fontSize:12, color:L.textMuted, marginTop:3 }}>Income vs expenses over time</div>
              </div>
            ...
            {!hasData && (
              <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', textAlign:'center', zIndex:2, pointerEvents:'none' }}>
                <div style={{ fontSize:13, fontWeight:600, color:L.textMuted }}>Upload documents to see your financial trends</div>
                <div style={{ fontSize:11, color:L.textFaint, marginTop:4 }}>Charts update automatically after each upload</div>
              </div>
            )}
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={chartData} margin={{ top:4, right:4, left:-16, bottom:0 }}>
                ...
                <XAxis dataKey="month" tick={{ fill:L.textMuted, fontSize:11 }} axisLine={false} tickLine={false}/>
                <YAxis tick={{ fill:L.textMuted, fontSize:11 }} axisLine={false} tickLine={false} tickFormatter={v => `$${v}`}/>
          ...
          <div style={{ ...card, padding:'22px 24px' }}>
            ...
            {!hasPie ? (
              <EmptyState icon={Receipt} title="No expense categories yet" description="Upload receipts and invoices to see your spending breakdown" action="Upload document" onAction={() => window.location.href = '/documents'} onAsk={() => askWithData('What expense categories should I be tracking?')} askLabel="Ask AI what to track"/>
            ) : (
              <>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8, marginTop:10 }}>
                  {pieData.slice(0, 4).map((d, i) => (
                    <div key={d.name} onClick={() => askWithData(`Tell me about my ${d.name} expenses. How can I reduce this?`)}
                      style={{ display:'flex', alignItems:'center', gap:8, cursor:'pointer', padding:'4px 6px', borderRadius:6, transition:'background 0.1s' }}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:14 }}>
          <div style={{ ...card, overflow:'hidden' }}>
            <div style={{ padding:'16px 22px', borderBottom:`1px solid ${L.border}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div>
                <div style={{ fontSize:14, fontWeight:700, color:L.text }}>Recent Transactions</div>
                <div style={{ fontSize:12, color:L.textMuted, marginTop:2 }}>{hasTxns ? `${txns.length} most recent entries` : 'No transactions yet'}</div>
              </div>
              <div style={{ display:'flex', gap:8 }}>
                {hasTxns && (
            ...
            {loading ? (
              ...
              <EmptyState icon={TrendingUp} title="No transactions yet" description="Upload an invoice or receipt and AI will automatically create transactions" action="Upload document" onAction={() => window.location.href = '/documents'} onAsk={() => askAndOpen('How do I add transactions to LedgerFlow?')} askLabel="How do I add transactions?"/>
            ) : (
              txns.map((t, i) => (
                <div key={t.id} onClick={() => askWithData(`Tell me about: ${t.vendor} $${Math.abs(t.amount)} on ${t.txn_date}. Is it deductible?`)}
                  style={{ display:'grid', gridTemplateColumns:'1fr 110px 130px 90px', padding:'12px 22px', borderBottom: i < txns.length - 1 ? `1px solid ${L.pageBg}` : 'none', alignItems:'center', transition:'background 0.1s', cursor:'pointer' }}
                  onMouseEnter={e => e.currentTarget.style.background = L.pageBg}
                  onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                  <div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                {[
                  ...
                  <button key={item.q} onClick={() => askWithData(item.q)}
                    style={{ padding:'8px 12px', borderRadius:L.radiusSm, background:L.pageBg, border:`1px solid ${L.border}`, color:L.textSub, cursor:'pointer', fontSize:12, fontFamily:L.font, textAlign:'left', transition:'all 0.15s', display:'flex', alignItems:'center', gap:8 }}
            ...
            {stats?.uncategorized > 0 && (
              <div onClick={() => askWithData(`I have ${stats.uncategorized} uncategorized transactions. Help me categorize them.`)}
                style={{ padding:'12px 14px', borderRadius:L.radiusSm, background:'rgba(245,158,11,0.08)', border:'1px solid rgba(245,158,11,0.2)', display:'flex', alignItems:'center', gap:10, cursor:'pointer', transition:'all 0.15s' }}
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:11, color:L.textMuted, marginTop:2 }}>Click to ask AI for help categorizing</div>
                </div>
            ...
            {(companyName || userEmail) && (
              <div style={{ ...card, padding:'14px 16px' }}>
                ...
                <button onClick={() => window.location.href = '/company-profile'}
                  style={{ marginTop:10, width:'100%', padding:'7px', borderRadius:L.radiusSm, background:L.pageBg, border:`1px solid ${L.border}`, color:L.textMuted, cursor:'pointer', fontSize:11, fontFamily:L.font, transition:'all 0.15s' }}
