...
const MENUS = {
  "Get Started": [{ label: "Get Started hub", href: "#" }, { label: "Onboarding programs", href: "#" }],
  "Topics": [
    { label: "Account management", href: "#" }, { label: "Accounting", href: "#" },
    { label: "Banking", href: "#" }, { label: "Expenses and suppliers", href: "#" },
    { label: "Get started", href: "#" }, { label: "Invoices and payments", href: "#" },
    { label: "Payroll", href: "#" }, { label: "Reports", href: "#" },
    { label: "Sales and customers", href: "#" }, { label: "Taxes", href: "#" },
    { label: "View all help", href: "#" }, { label: "Year End Resources", href: "#" },
  ],
  "Training": [{ label: "Tutorials (Videos)", href: "#" }, { label: "Webinars", href: "#" }, { label: "Novala Courses", href: "#" }],
  "Community": [
    { label: "Community Home", href: "#" }, { label: "Novala Q&A", href: "#" },
    { label: "Community Basics", href: "#" }, { label: "Champions Program", href: "#" },
    { label: "Ask the Community", href: "#" },
  ],
  "Resources": [
    { label: "Find an Expert", href: "#" }, { label: "Novala Blog", href: "#" },
    { label: "Product Updates", href: "#" }, { label: "Novala Status page", href: "#" },
...
function LocaleIcon({ locale, large }) {
  if (locale.flagSrc) {
    ...
    return <img src={locale.flagSrc} alt={locale.label} style={{ ...dims, objectFit: "cover", borderRadius: 2, border: "1px solid rgba(0,0,0,0.1)", display: "block", flexShrink: 0 }} />;
  }
  return <Globe size={large ? 22 : 18} strokeWidth={1.8} style={{ flexShrink: 0, color: SUB }} />;
}

function NavMenu({ label, items, openMenu, setOpenMenu }) {
  const isOpen = openMenu === label;
  return (
    <div onMouseEnter={() => setOpenMenu(label)} onMouseLeave={() => setOpenMenu(null)} style={{ position: "relative" }}>
      <button style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 4, color: INK, fontWeight: 500, fontSize: 14, fontFamily: "inherit", padding: "6px 8px", borderBottom: isOpen ? `2px solid ${TEAL}` : "2px solid transparent", transition: "border-color 0.15s" }}>
        {label}
        {isOpen ? <ChevronUp size={14} strokeWidth={2} /> : <ChevronDown size={14} strokeWidth={2} />}
      </button>
      {isOpen && (
        <div style={{ position: "absolute", top: "100%", left: 0, background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 10, minWidth: 220, padding: "8px 0", boxShadow: "0 10px 30px -10px rgba(0,0,0,0.18)", zIndex: 50 }}>
          {items.map((it, i) => (
            <a key={i} href={it.href} onMouseEnter={(e) => { e.currentTarget.style.background = "#F1F5F5"; }} onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; }} style={{ display: "block", padding: "10px 16px", color: INK, textDecoration: "none", fontSize: 14 }}>{it.label}</a>
          ))}
        </div>
      )}
    </div>
...
function LocaleRow({ locale, isSelected, onClick }) {
  return (
    <button onClick={onClick} onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.background = "#F9FAFA"; }} onMouseLeave={(e) => { if (!isSelected) e.currentTarget.style.background = "#fff"; }} style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "10px 14px", minHeight: 42, background: isSelected ? "#F1F5F5" : "#fff", border: "none", cursor: "pointer", fontSize: 14, color: INK, fontFamily: "inherit", textAlign: "left" }}>
  ...
  return (
    <div>
      ...
      <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 10 }}>
        {items.map((it, i) => (<li key={i}><a href="#" style={{ color: "rgba(255,255,255,0.78)", textDecoration: "none", fontSize: 14 }}>{it}</a></li>))}
      </ul>
    </div>
  );
}
...
export default function HelpAutoPayroll() {
  ...
  const selectLocale = (code) => { setLocale(code); setShowNavLocale(false); setShowFooterLocale(false); };
  const handleSearch = (e) => { e.preventDefault(); const q = search.trim(); if (!q) return; navigate(`/help/search?q=${encodeURIComponent(q)}`); };

  const supportLinkStyle = { fontSize: 18, fontWeight: 700, color: INK, textDecoration: "none", borderBottom: hoverSupport ? `2px solid ${TEAL}` : "2px solid transparent", paddingBottom: 4, transition: "border-color 0.15s" };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", color: INK }}>
      <div style={{ borderBottom: `1px solid ${BORDER}`, padding: "12px 32px" }}>
        <div style={{ maxWidth: CONTENT_MAX, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 14 }}>
          <a href="/" style={{ textDecoration: "none", display: "inline-flex", alignItems: "center" }}><span style={{color: "#0F9599", fontWeight: 800, fontSize: 24, letterSpacing: "-0.015em", fontFamily: "inherit"}}>Novala</span></a>
          <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
            ...
            <span style={{ color: SUB, display: "flex", alignItems: "center", gap: 6 }}><Phone size={14} strokeWidth={2} /><span>1-800-NOVALA</span></span>
            <a href="/login" style={{ padding: "8px 18px", borderRadius: 8, background: "#fff", color: LINK_BLUE, border: `1.5px solid ${LINK_BLUE}`, fontWeight: 600, fontSize: 14, textDecoration: "none" }}>Sign In</a>
          </div>
        </div>
      </div>
      ...
      <div style={{ borderBottom: `1px solid ${BORDER}`, padding: "16px 32px", position: "sticky", top: 0, zIndex: 40, background: "#fff" }}>
        <div style={{ maxWidth: CONTENT_MAX, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div ref={navLocaleRef} style={{ position: "relative" }}>
            ...
            {showNavLocale && (
              <div style={{ position: "absolute", top: "calc(100% + 6px)", right: 0, background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 10, minWidth: 240, boxShadow: "0 10px 30px -10px rgba(0,0,0,0.18)", zIndex: 50, overflow: "hidden" }}>
                {NAV_LOCALES.map(l => (<LocaleRow key={l.code} locale={l} isSelected={l.code === locale} onClick={() => selectLocale(l.code)} />))}
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{ background: BG_PROMO, padding: "20px 32px" }}>
        <div style={{ maxWidth: CONTENT_MAX, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 24, fontSize: 15 }}>
          <span style={{ color: INK }}>Explore our Novala training videos and learn at your own pace</span>
          <button style={{ padding: "10px 22px", borderRadius: 8, background: TEAL, color: "#fff", border: "none", fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "inherit", flexShrink: 0, boxShadow: "0 6px 14px -6px rgba(15,149,153,0.6)" }}>Master Novala today</button>
        </div>
      </div>

      <div style={{ background: `linear-gradient(135deg, ${TEAL} 0%, ${TEAL_DARK} 100%)`, padding: "56px 32px" }}>
        <div style={{ maxWidth: CONTENT_MAX, margin: "0 auto" }}>
          <form onSubmit={handleSearch} style={{ display: "flex", width: "100%", maxWidth: 560, background: "#fff", borderRadius: 12, overflow: "hidden", boxShadow: "0 10px 30px -10px rgba(0,0,0,0.25)" }}>
            <input type="text" name="q" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search questions, keywords or topics" style={{ flex: 1, padding: "14px 18px", border: "none", outline: "none", fontSize: 15, fontFamily: "inherit", color: INK }} />
            <button type="submit" style={{ padding: "0 24px", background: TEAL, color: "#fff", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ maxWidth: CONTENT_MAX, margin: "0 auto", padding: "48px 32px 80px", display: "grid", gridTemplateColumns: "1fr 280px", gap: 60 }}>
        <article>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
            <div style={{ width: 32, height: 32, borderRadius: 999, background: TEAL, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 13 }}>N</div>
            <span style={{ color: SUB, fontSize: 13, fontWeight: 600 }}>Novala Help</span>
          </div>
          <h1 style={{ margin: "0 0 12px 0", fontSize: 32, fontWeight: 700, letterSpacing: "-0.01em" }}>Set up and manage your Auto Payroll</h1>
          <div style={{ color: SUB, fontSize: 14, marginBottom: 28 }}>by Novala · Updated a few days ago</div>
          <p style={{ fontSize: 16, lineHeight: 1.65, color: INK, marginBottom: 18 }}>Novala Payroll saves time by automatically creating pay (cheques or direct deposits) for salaried and hourly employees who work a set number of hours. Auto Payroll works for employees paid by direct deposit or paper cheque.</p>
          ...
          <Section title="Add or remove eligible employees from Auto Payroll">
            ...
            <p style={{ background: "#F1F5F5", padding: "12px 16px", borderRadius: 8, fontSize: 14, marginTop: 14 }}>Once enrolled, you'll get an email <strong>2 business days before</strong> Auto Payroll processes, and a confirmation email <strong>after</strong>.</p>
          </Section>
      ...
      <footer style={{ background: FOOTER_DARK, color: "#fff", padding: "48px 32px 32px" }}>
        <div style={{ maxWidth: CONTENT_MAX, margin: "0 auto" }}>
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.15)", paddingTop: 24, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 16, fontSize: 13, color: "rgba(255,255,255,0.78)" }}>
            ...
            <div ref={footerLocaleRef} style={{ position: "relative" }}>
              ...
              {showFooterLocale && (
                <div style={{ position: "absolute", bottom: "calc(100% + 6px)", right: 0, background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 10, minWidth: 240, boxShadow: "0 10px 30px -10px rgba(0,0,0,0.25)", zIndex: 50, overflow: "hidden" }}>
                  {FOOTER_LOCALES.map(l => (<LocaleRow key={l.code} locale={l} isSelected={l.code === locale} onClick={() => selectLocale(l.code)} />))}
                </div>
              )}
            </div>
          </div>
