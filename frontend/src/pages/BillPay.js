...
import {
  ...
  Trash2, X, DollarSign, Calendar, Tag,
  Building2, Sparkles, ArrowRight, FileText,
  CheckCircle, Filter,
} from 'lucide-react';
...
const STORAGE_KEY = 'novala_bills';
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:isMobile?'flex-end':'center', justifyContent:'center', zIndex:1000, backdropFilter:'blur(4px)' }}>
      <div style={{ background:'#fff', borderRadius:isMobile?'20px 20px 0 0':16, width:isMobile?'100%':500, maxWidth:isMobile?'100%':'90vw', maxHeight:isMobile?'90vh':'85vh', overflowY:'auto', boxShadow:'0 24px 60px rgba(0,0,0,0.2)' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'18px 24px', borderBottom:`1px solid ${L.border}`, position:'sticky', top:0, background:'#fff', zIndex:10 }}>
          ...
          <button onClick={onClose} style={{ background:'none', border:'none', cursor:'pointer', color:L.textMuted, display:'flex' }}>
            <X size={18}/>
          </button>
        </div>
...
export default function BillPay() {
  ...
  useEffect(() => {
    ...
    const unpaid   = bills.filter(b => getStatus(b) === 'unpaid').length;
    const totalOwed = bills.filter(b => b.status !== 'paid').reduce((s,b) => s + (b.amount||0), 0);
    setPageContext('billpay', {
      page:'billpay', total_bills:bills.length,
      overdue, unpaid, total_owed:totalOwed,
    });
  }, [bills]);
  ...
  const resetForm = () => {
    ...
    setCategory('General Expense'); setNotes(''); setRecurring(false);
  };
  ...
  const handleAddBill = () => {
    ...
    resetForm();
    setModal(null);
    showSuccess(`✓ Bill from ${newBill.vendor} added`);
  };
    ...
    try {
      ...
      await fetch(`${BASE}/transactions/`, {
        ...
        body:JSON.stringify({
          ...
          txn_type:  'expense',
          notes:     `Bill payment: ${bill.vendor}${bill.notes ? ` — ${bill.notes}` : ''}`,
        }),
      });
      ...
      // Even if API fails, mark as paid locally
      setBills(prev => prev.map(b => b.id === bill.id ? { ...b, status:'paid', paid_at:new Date().toISOString() } : b));
      showSuccess(`✓ ${bill.vendor} marked as paid`);
    } finally {
      setSaving(false);
      setModal(null);
    }
  ...
  const exportPDF = () => {
    ...
    doc.text(`Total bills: ${bills.length} · Overdue: ${bills.filter(b=>getStatus(b)==='overdue').length} · Total owed: ${fmt(bills.filter(b=>b.status!=='paid').reduce((s,b)=>s+b.amount,0))}`, 14, 40);

    autoTable(doc, {
      startY:48,
      head:[['Vendor','Amount','Due Date','Category','Status']],
      body:filtered.map(b => [
        b.vendor, fmt(b.amount), b.due_date, b.category, STATUS_STYLES[getStatus(b)]?.label||'—',
      ]),
      styles:{ fontSize:9 },
  ...
  const filtered = bills.filter(b => {
    ...
    if (filter === 'paid')    return getStatus(b) === 'paid';
    return true;
  }).sort((a,b) => {
  ...
  const totalOverdue = bills.filter(b => getStatus(b) === 'overdue').reduce((s,b) => s + b.amount, 0);
    ...
    <div style={page}>
      ...
      <div style={{ ...topBar, flexDirection:isMobile?'column':'row', alignItems:isMobile?'flex-start':'center', gap:isMobile?10:0, padding:isMobile?'16px':undefined }}>
        <div>
          ...
          <div style={{ fontSize:12, color:L.textMuted, marginTop:2 }}>Track bills, due dates and payments — auto-creates transactions</div>
        </div>
      ...
      <div style={{ padding: pad }}>
          ...
          <div style={{ padding:'12px 16px', borderRadius:L.radiusSm, background:'rgba(239,68,68,0.06)', border:'1px solid rgba(239,68,68,0.2)', marginBottom:16, display:'flex', alignItems:'center', gap:10 }}>
            ...
            <div style={{ flex:1 }}>
              <div style={{ fontSize:13, fontWeight:700, color:'#EF4444' }}>
                {overdueCount} overdue bill{overdueCount!==1?'s':''} — {fmt(totalOverdue)} past due
              </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:isMobile?'repeat(2,1fr)':'repeat(4,1fr)', gap:isMobile?10:14, marginBottom:20 }}>
          {[
            ...
            { label:'Paid This Month', value:fmt(totalPaid), color:ACCENT,  sub:'Completed payments'  },
          ].map(c => (
            <div key={c.label} style={{ ...card, padding:isMobile?'12px 14px':'18px 20px' }}>
              ...
              <div style={{ fontSize:isMobile?16:20, fontWeight:700, color:c.color, fontFamily:L.fontMono, marginBottom:4, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{c.value}</div>
        ...
        <div style={{ ...card, overflow:'hidden' }}>
          ...
          {!isMobile && filtered.length > 0 && (
            <div style={{ display:'grid', gridTemplateColumns:'1fr 120px 120px 160px 120px 180px', padding:'8px 22px', borderBottom:`1px solid ${L.border}`, background:L.pageBg }}>
          ...
          {filtered.map(bill => {
                ...
                <div key={bill.id} style={{ padding:'14px 16px', borderBottom:`1px solid ${L.border}` }}>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10, flexWrap:'wrap' }}>
                    ...
                    <span style={{ fontSize:10, color:L.textMuted, display:'flex', alignItems:'center', gap:4 }}>
                      ...
                      {status!=='paid' && (
                        <span style={{ color:days<0?'#EF4444':days<=3?'#F59E0B':L.textMuted }}>
            ...
            return (
              <div key={bill.id}
                style={{ display:'grid', gridTemplateColumns:'1fr 120px 120px 160px 120px 180px', padding:'13px 22px', borderBottom:`1px solid ${L.border}`, alignItems:'center', transition:'background 0.1s' }}
                ...
                {/* Amount */}
                <div style={{ fontSize:13, fontWeight:700, fontFamily:L.fontMono, color:status==='paid'?ACCENT:status==='overdue'?'#EF4444':'#F59E0B' }}>
                  {fmt(bill.amount)}
                </div>
                {/* Due date */}
                <div>
          ...
          <div style={{ ...card, padding:isMobile?'16px':'20px 24px', marginTop:20 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)', gap:8 }}>
              {[
                ...
                { q:'How much cash do I need to pay all overdue bills?',  icon:'💰' },
      ...
      {modal === 'add' && (
        <Modal title="Add New Bill" onClose={() => setModal(null)}>
          <InputField label="Vendor / Payee" required placeholder="e.g. AWS, Office Landlord" value={vendor} onChange={e => setVendor(e.target.value)}/>
          <InputField label="Amount" required type="number" placeholder="0.00" value={amount} onChange={e => setAmount(e.target.value)}/>
          <InputField label="Due Date" required type="date" value={dueDate} onChange={e => setDueDate(e.target.value)}/>
          ...
          <InputField label="Notes (optional)" placeholder="Reference number, invoice #, etc." value={notes} onChange={e => setNotes(e.target.value)}/>
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            <button onClick={handleAddBill} disabled={!vendor.trim()||!amount||!dueDate}
              style={{ flex:1, padding:'10px', borderRadius:L.radiusSm, background:!vendor.trim()||!amount||!dueDate?L.textFaint:ACCENT, color:'#fff', border:'none', cursor:!vendor.trim()||!amount||!dueDate?'not-allowed':'pointer', fontSize:13, fontWeight:600, fontFamily:L.font, boxShadow:'0 4px 12px rgba(10,185,138,0.3)' }}>
              Add Bill
            </button>
          </div>
        </Modal>
      ...
      {modal === 'pay' && selectedBill && (
        <Modal title="Mark Bill as Paid" onClose={() => setModal(null)}>
          ...
          <div style={{ padding:'12px 14px', borderRadius:L.radiusSm, background:L.accentSoft, border:`1px solid ${L.accentBorder}`, fontSize:12, color:L.textSub, marginBottom:20, display:'flex', alignItems:'flex-start', gap:8 }}>
            ...
            Marking this bill as paid will automatically create an expense transaction in your books.
          </div>
      ...
      {modal === 'delete' && selectedBill && (
        <Modal title="Delete Bill" onClose={() => setModal(null)}>
          <div style={{ textAlign:'center', padding:'16px 0 24px' }}>
            ...
            <div style={{ fontSize:13, color:L.textMuted }}>This will remove the bill. This cannot be undone.</div>
          </div>
          <div style={{ display:'flex', gap:8 }}>
            <button onClick={() => setModal(null)}
              style={{ flex:1, padding:'10px', borderRadius:L.radiusSm, background:'transparent', border:`1px solid ${L.border}`, color:L.textMuted, cursor:'pointer', fontSize:13, fontFamily:L.font }}>
